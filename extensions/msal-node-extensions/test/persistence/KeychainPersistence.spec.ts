...
import { KeychainPersistence } from "../../src";
...
describe('Test KeyChainPersistence', () => {
    ...
    test('exports a class', async () => {
        const persistence = await KeychainPersistence.create(filePath, serviceName, accountName);
        expect(persistence).toBeInstanceOf(KeychainPersistence);
    });

    test('creates a cache persistence if doesnt exist', async () => {
        await KeychainPersistence.create(filePath, serviceName, accountName);
