...
For web applications, use the cache plugin with an appropriate partitioning and persistence strategy for the application's scale and security requirements.
...
PersistenceCreator.createPersistence(persistenceConfiguration).then(
    async (persistence) => {
        ...
        // Use the confidential client application as required...
    }
);
```
