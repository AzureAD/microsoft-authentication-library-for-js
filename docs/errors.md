...
-   `popup_relay_invalid_allowed_origin` - An entry in the `allowedAuthorityOrigins` option could not be parsed as a URL. Fix the malformed entry; entries are rejected rather than silently dropped so the allow list cannot change size unnoticed.
