...
Now decide if you want your feature or bug fix to go into the dev branch or the v4-lts branch. **All bug fixes and new features should go into the dev branch.**

The v4-lts branch is effectively frozen; patches that change the SDKs protocols or API surface area or affect the run-time behavior of the SDK will be rejected.
...
Generates a changefile based on the `v4-lts` branch. This should be run on your PR before merging to `v4-lts`, **AFTER** you have created your PR. When this command is run, the type of change needs to be selected:
...
Bumps versions for packages with relevant changefiles. Run on `v4-lts` branch to check which packages are slated for release.
