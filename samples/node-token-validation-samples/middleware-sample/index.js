...
        piiLoggingEnabled: false,
    ...
    const tokenValidator = new nodeTokenValidation.TokenValidator(validationConfig);
    ...
    app.get("/redirect", (req, res) => {
        ...
        clientApplication.acquireTokenByCode(tokenRequest, authCodeResponse).then((response) => {
            ...
            console.log("ACQUIRE TOKEN RESPONSE: ", response);
            // console.log("REDIRECT GET?: ", req);
            if (!req.headers.authorization) {
                req.headers.authorization = `Bearer ${response.accessToken}`;
            }
            console.log("REDIRECT REQ HEADERS: ", req.headers);
    ...
    const tokenValidationParams = {
        validIssuers: ["https://login.microsoftonline.com/common/v2.0"],
        validAudiences: ["aud"],
        nonce: "nonce",
        subject: "sub"
    };

    app.get("/graph", tokenValidator.validateTokenMiddleware(tokenValidationParams), (req, res) => {
        console.log("GRAPH REQUEST: ", req);
