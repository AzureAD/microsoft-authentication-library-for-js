...
describe("Native Auth Sample - Sign In Tests", () => {
    ...
    describe("Sign In Flow - Email + Password", () => {
        ...
        it("User signs in with account A when account A has already signed in", async () => {
            ...
            expect(errorMessage).toContain("Error: user_already_signed_in:");
        }, AUTH_TIMEOUT);
        ...
        it("User enters non registered username", async () => {
    ...
    describe("Sign In Flow - Email + OTP", () => {
        ...
        it("User email is registered with email OTP auth method, which is supported by the developer (unfinished)", async () => {
            ...
            await page.waitForSelector("#codeVerificationCard", { visible: true });
            ...
            // Wait for the submit button to be visible and enabled
            await page.waitForSelector("#submitCodeBtn:enabled", { visible: true, timeout: 15000 });
            // cannot obtain otp code from email, so just verify the submit button is enabled
            await screenshot.takeScreenshot(page, "otpInputReadyForSubmission");
        }, AUTH_TIMEOUT);
    });
