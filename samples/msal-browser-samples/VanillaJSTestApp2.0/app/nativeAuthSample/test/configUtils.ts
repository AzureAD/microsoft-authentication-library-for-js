    ...
    private static loadConfig(): any {
        if (!this.configCache) {
            try {
                ...
                throw new Error(
                    `Failed to load native auth config from ${this.CONFIG_PATH}: ${error}`
                );
            }
        }
    ...
    static getValue(key: string): any {
        ...
        for (const k of keys) {
            if (value && typeof value === "object" && k in value) {
                ...
                throw new Error(
                    `Key '${key}' not found in native auth configuration`
                );
            }
        }
...
export const nativeAuthConfig = {
    emailPasswordClientId: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.EMAIL_PASSWORD_CLIENT_ID
    ),
    emailCodeClientId: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.EMAIL_CODE_CLIENT_ID
    ),
    emailPasswordAttributesClientId: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.EMAIL_PASSWORD_ATTRIBUTES_CLIENT_ID
    ),
    emailCodeAttributesClientId: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.EMAIL_CODE_ATTRIBUTES_CLIENT_ID
    ),
    tenantSubdomain: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.TENANT_SUBDOMAIN
    ),
    tenantId: getNativeAuthConfigValue(NATIVE_AUTH_CONFIG_KEYS.TENANT_ID),
    signInEmailPasswordUsername: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.SIGN_IN_EMAIL_PASSWORD_USERNAME
    ),
    signInEmailPasswordUsernameMfa: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.SIGN_IN_EMAIL_PASSWORD_USERNAME_MFA
    ),
    signInEmailCodeUsername: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.SIGN_IN_EMAIL_CODE_USERNAME
    ),
    resetPasswordUsername: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.RESET_PASSWORD_USERNAME
    ),
    passwordSignInEmailCode: getNativeAuthConfigValue(
        NATIVE_AUTH_CONFIG_KEYS.PASSWORD_SIGN_IN_EMAIL_CODE
    ),
    passwordProvider: getNativeAuthConfigValue(
