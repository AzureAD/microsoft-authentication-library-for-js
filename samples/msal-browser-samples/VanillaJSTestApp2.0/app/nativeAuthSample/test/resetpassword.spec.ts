...
describe("Native Auth Sample - Reset Password Tests", () => {
    ...
    describe("Reset Password Flow - Email + Password - Positive", () => {
        ...
        it("User requests reset inputs emails, receives code, sets new valid password, completes reset, auto-signs in", async () => {
            ...
            console.log("Retrieving OTP code from email for reset password...");
            ...
            // Submit the OTP code
            await page.evaluate(() => {
                const submitButton = document.getElementById("submitResetPasswordCodeBtn");
                if (submitButton) {
                    submitButton.click();
                } else {
            ...
            await screenshot.takeScreenshot(page, "newPasswordInputDisplayed");

            // Enter new password
            await page.waitForSelector("#resetPasswordNewPassword", {
                visible: true,
            });
            await page.type("#resetPasswordNewPassword", nativeAuthConfig.passwordSignInEmailCode);
            await screenshot.takeScreenshot(page, "newPasswordEntered");

            // Submit new password
            await page.waitForSelector("#submitResetPasswordNewPasswordBtn:enabled", {
                visible: true,
                timeout: 15000,
            });

            await page.evaluate(() => {
                const submitButton = document.getElementById("submitResetPasswordNewPasswordBtn");
                if (submitButton) {
                    submitButton.click();
                } else {
                    throw new Error("Submit new password button not found in the DOM");
                }
            });
            ...
            await page.waitForFunction(
                () => {
                    ...
                    const isCompleted = authStatusBanner && 
                        (authStatusBanner.textContent?.includes("Password reset completed") ||
            ...
            // Check if user was automatically signed in or just completed reset
            if (authStatusBanner.includes("Signed in")) {
                // Verify tokens and authentication if automatically signed in
                ...
                // Just verify password reset completed successfully
                expect(authStatusBanner).toContain("reset completed");
            }
    ...
    describe("Reset Password Flow - Email + Password - Negative", () => {
        ...
        it("User submits non-existing email, receives account not found error", async () => {
            ...
            await page.waitForSelector("#errorBanner", { visible: true, timeout: 20000 });
        ...
        it("User submits existing email, but email does not linked to any password (registered as email + OTP)", async () => {
        ...
        it("User submits existing email, and submit incorrect code", async () => {
        ...
        it("User submits existing email, receives code, creates invalid password (doesn't meet password complexity requirements), receives requirements error", async () => {
            ...
            await page.type("#resetPasswordNewPassword", testData.invalidPassword);
            ...
            await screenshot.takeScreenshot(page, "passwordComplexityErrorDisplayed");
            ...
            const newPasswordCard = await page.$("#resetPasswordNewPasswordCard");
            expect(newPasswordCard).toBeTruthy();

            const isVisible = await page.evaluate(() => {
                const card = document.getElementById("resetPasswordNewPasswordCard");
                return (
