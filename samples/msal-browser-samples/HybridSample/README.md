...
It shows how to use two new APIs, `returnSpaCode` in MSAL Node and `acquireTokenByCode` in MSAL.js v2, to authenicate a user server-side using a confidential client, and then SSO that user client-side using a second authorization code that is returned to the confidential client and redeemed by the public client client-side. This helps mitigate user experience and performance concerns that arise when performing server-side and client-side authenication for the same user, especially when third-party cookies are blocked by the browser.
...
When invoking this API, set `returnSpaCode` to `true`, which will enable MSAL to acquire a second authorization code to be redeemed by your single-page application. 
