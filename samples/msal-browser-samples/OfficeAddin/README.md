...
1. `./src/taskpane/taskpane.html` - Main taskpane UI with Office Fluent UI styling
...
1. `./src/commands/commands.js` - Office Add-in command functions
1. `./webpack.config.js` - Webpack configuration for bundling the add-in
1. `./package.json` - Project dependencies including MSAL Browser and Office development tools

### Key Features Demonstrated
- **Nested App Authentication (NAA)**: Uses `createNestablePublicClientApplication()` for embedded scenarios
- **Silent Token Acquisition**: Demonstrates `acquireTokenSilent()` for seamless user experience
...
- Install Office development tools: `npm install -g @microsoft/office-addin-cli`
...
```javascript
// Ensure Office.js is ready before initializing MSAL
Office.onReady((info) => {
    if (info.host === Office.HostType.Word) {
        // Initialize your authentication flow
        initializeMsal();
    }
});
```
...
1. **Nested Authentication**: Uses `createNestablePublicClientApplication()` instead of standard `createPublicClientApplication()`
1. **Embedded Context**: Runs within Office's security and UI constraints
1. **Manifest-based Deployment**: Requires XML manifest for Office platform integration
1. **Office.js Dependency**: Must integrate with Office.js APIs for proper functionality
...
1. User profile information will be displayed in the taskpane if successful
1. Any authentication errors will be logged to the browser console
