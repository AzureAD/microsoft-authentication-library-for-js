...
describe("Native Auth Sample - Sign Up Tests", () => {
    ...
    describe("Sign Up Flow - Email + Password - Positive", () => {
        ...
        it("User inputs new email, enters incorrect OTP, resends code, verifies correct code, creates password, completes signup with auto sign-in", async () => {
            ...
            const emailClient = new MailTmClient(nativeAuthConfig.passwordProvider);
            const { address: signUpEmail } = await emailClient.createInbox();
            console.log(`Created email for positive signup: ${signUpEmail}`);
            ...
            await screenshot.takeScreenshot(page, "2_otpVerificationDisplayed");
            ...
            await screenshot.takeScreenshot(page, "4_incorrectOtpSubmitted");
            ...
            await screenshot.takeScreenshot(page, "10_passwordInputDisplayed");
            ...
            await screenshot.takeScreenshot(page, "13_automaticSignInCompleted");
            ...
            const finalAuthStatus = await page.$eval("#authStatusBanner", (el) => el.textContent);
    ...
    describe("Sign Up Flow - Email + Password - Negative", () => {
        ...
        it("User enters username, attributes to start sign-up flow, and enter the incorrect otp", async () => {
            ...
            console.log(`Using mock email for negative signup test: ${signUpEmail}`);
        ...
        it("User inputs new email, verifies code, creates invalid password (does not meet requirements), receives sign up error", async () => {
            ...
            const emailClient = new MailTmClient(nativeAuthConfig.passwordProvider);
            const { address: signUpEmail } = await emailClient.createInbox();
            console.log(`Created email for invalid password test: ${signUpEmail}`);
            ...
            if (screenshot) {
                await screenshot.takeScreenshot(page, "5_passwordInputDisplayed");
            }
            ...
            if (screenshot) {
                await screenshot.takeScreenshot(page, "7_invalidPasswordSubmitted");
            }

            // Phase 7: Wait for error banner to appear
            await page.waitForSelector("#errorBanner", { visible: true, timeout: 15000 });
            if (screenshot) {
                await screenshot.takeScreenshot(page, "8_passwordErrorBannerDisplayed");
            }

            // Phase 8: Verify error banner content for password validation
            const errorMessage = await page.$eval("#errorMessage", (el) => el.textContent);
            // Expect password requirement error message
            expect(errorMessage).toMatch(/(password|requirement|length|complexity|invalid)/i);
        }, AUTH_TIMEOUT);
        ...
        it("User signs in with existing email, then tries to sign up with same email, receives error to sign out first", async () => {
            ...
            await page.type("#username", nativeAuthConfig.signInEmailPasswordUsername);
            ...
            await page.type("#signInPassword", nativeAuthConfig.passwordSignInEmailCode);
            await page.evaluate(() => {
            ...
            await page.type("#signUpFirstName", testFirstName);
            await page.type("#signUpLastName", testLastName);
            await page.type("#signUpUsername", nativeAuthConfig.signInEmailPasswordUsername); // Same email
            if (screenshot) {
            ...
            await page.waitForSelector("#errorBanner", { visible: true, timeout: 15000 });
            if (screenshot) {
                await screenshot.takeScreenshot(page, "6_errorBannerDisplayed");
            }

            // Verify error banner content - expect message about being already signed in
            const errorMessage = await page.$eval("#errorMessage", (el) => el.textContent);
            expect(errorMessage).toMatch(/(sign out|already signed|logged in|user already signed)/i);
        }, AUTH_TIMEOUT);
    ...
    describe("Sign Up Flow - Email + OTP - Positive", () => {
        ...
        it("User enters new email and user attributes, verifies code successfully, completes sign up flow, then automatically sign-in", async () => {
            ...
            await screenshot.takeScreenshot(page, "5_automaticSignInCompleted");
        ...
        it("User enters new email and user attributes, uses invalid OTP, requests new code, completes sign up flow, then automatically sign-in", async () => {
            ...
            const emailClient = new MailTmClient(nativeAuthConfig.passwordProvider);
            const { address: signUpEmail } = await emailClient.createInbox();
            console.log(`Created email for OTP signup with resend: ${signUpEmail}`);
            ...
            await screenshot.takeScreenshot(page, "10_automaticSignInCompleted");
