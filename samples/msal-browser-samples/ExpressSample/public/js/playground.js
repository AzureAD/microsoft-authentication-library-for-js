...
function setupEventListeners() {
    ...
    document.getElementById('resetConfig')?.addEventListener('click', loadDefaultConfiguration);
    document.getElementById('applyConfig')?.addEventListener('click', applyConfiguration);

    // API action buttons
    document.getElementById('btnAcquireTokenRedirect')?.addEventListener('click', () => handleApiCall('acquireTokenRedirect'));
    document.getElementById('btnAcquireTokenPopup')?.addEventListener('click', () => handleApiCall('acquireTokenPopup'));
    document.getElementById('btnAcquireTokenSilent')?.addEventListener('click', () => handleApiCall('acquireTokenSilent'));
...
function loadDefaultConfiguration() {
    ...
    const defaultConfig = {
        auth: {
            ...
            authority: currentConfig.auth.authority || "https://login.microsoftonline.com/common",
            redirectUri: currentConfig.auth.redirectUri || window.location.origin,
        },
        cache: {
    ...
    showSuccess('Default configuration loaded. Click "Apply Configuration" when ready.');
}
...
async function applyConfiguration() {
    try {
        ...
        const redirectResponse = await playgroundMsalInstance.handleRedirectPromise();
        ...
        if (redirectResponse) {
            ...
            if (playgroundMsalInstance.getAllAccounts().length > 0 && !playgroundMsalInstance.getActiveAccount()) {
                playgroundMsalInstance.setActiveAccount(playgroundMsalInstance.getAllAccounts()[0]);
            }
        ...
        displayResponse({
            ...
            details: 'Please check your JSON syntax and configuration values',
            timestamp: new Date().toISOString()
        }, 'error');
    }
}
    ...
    try {
        ...
        switch (apiName) {
            ...
            case 'acquireTokenPopup':
                result = await playgroundMsalInstance.acquireTokenPopup(request);
                displayResponse({
                    api: apiName,
                    result: result,
                    timestamp: new Date().toISOString()
            ...
            case 'acquireTokenSilent':
                ...
                result = await playgroundMsalInstance.acquireTokenSilent(request);
                displayResponse({
                    api: apiName,
                    result: result,
                    timestamp: new Date().toISOString()
