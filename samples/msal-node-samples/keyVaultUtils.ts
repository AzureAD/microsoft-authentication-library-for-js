...
export const getKeyVaultSecretClient = (): Promise<void> => {
    return new Promise<void>(async (resolve, reject) => {
        ...
        const vaultName = "msaljs-key-vault";
        const url = `https://${vaultName}.vault.cloudapi.microsoft.${process.env.TLD}/`;
        try {
            const client = await new SecretClient(url, keyVaultCredentials);
            resolve(client);
        } catch (error) {
            reject(error);
        }
