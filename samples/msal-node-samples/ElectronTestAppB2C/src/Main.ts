...
export default class Main {
    ...
    private static async callApi(): Promise<void> {
        console.log("Call API");
        const token = await Main.authProvider.getApiToken();
        const response = await Main.networkModule.callEndpointWithToken("https://fabrikamb2chello.azurewebsites.net/hello", token);
    ...
    private static registerSubscriptions(): void {
        ...
        ipcMain.on(IPC_MESSAGES.CALL_API, Main.callApi);
