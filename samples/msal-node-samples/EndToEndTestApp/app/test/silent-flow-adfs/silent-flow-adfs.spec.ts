...
//                 await page.waitForSelector("#acquireTokenSilent");
//                 const originalAccessToken = await NodeCacheTestUtils.getAccessTokens(TEST_CACHE_LOCATION)[0].token;
//                 NodeCacheTestUtils.expireAccessTokens(TEST_CACHE_LOCATION);
//                 const expiredAccessToken = await NodeCacheTestUtils.getAccessTokens(TEST_CACHE_LOCATION)[0].token;
//                 await page.click("#acquireTokenSilent");
//                 await page.waitForSelector(`#${SUCCESSFUL_GRAPH_CALL_ID}`);
//                 const refreshedAccessToken = await NodeCacheTestUtils.getAccessTokens(TEST_CACHE_LOCATION)[0].token;
//                 await screenshot.takeScreenshot(page, "acquireTokenSilentGotTokens");
