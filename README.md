...
userAgentApplication.loginPopup(['user.read']).then(
  token => {
    // succeed
...
```js
// get an access token
userAgentApplication.acquireTokenSilent(['user.read']).then(
  token => { /* ATS promise resolved */ },
  error => {
    if (error.indexOf('interaction_required') !== -1) {
      userAgentApplication.acquireTokenPopup(['user.read']).then(
        token => { /* succeed */ },
        error => { /* failed */ }
      );
    }
