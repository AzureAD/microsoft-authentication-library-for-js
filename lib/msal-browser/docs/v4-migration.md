...
- **Redirect flow**: The redirect bridge caches the authentication response in `sessionStorage` and navigates back to your application's page that initiated the redirect. On the destination page, `handleRedirectPromise` picks up the cached response automatically. This avoids appending auth parameters to the URL, which would cause issues for single-page applications that use hash-based routing (e.g., `/#/dashboard`).
