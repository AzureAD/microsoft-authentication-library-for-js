...
- **Redirect flow**: The redirect bridge typically caches the authentication response in `sessionStorage` and navigates back to your application's page that initiated the redirect. On the destination page, `handleRedirectPromise` picks up the cached response automatically. This avoids appending auth parameters to the URL in the common case, which would otherwise cause issues for single-page applications that use hash-based routing (e.g., `/#/dashboard`). In rare cases where MSAL cannot cache the response (for example, if the interaction state is missing or malformed, or if `sessionStorage` writes fail due to quota limits or restricted storage), MSAL falls back to appending the auth response parameters to the navigation URL. If your app relies on hash-based routing, ensure that your router can handle this fallback without breaking navigation or leaking sensitive information.
