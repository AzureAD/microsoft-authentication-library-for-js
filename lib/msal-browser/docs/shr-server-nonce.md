...
As an enhancement of Access Token Proof-of-Possesion, MSAL Browser provides a way to insert a server-generated signed timestamp (a.k.a **server nonce**) into a `Signed HTTP Request`, also known as a `PoP Token`. These server generated nonce can be added to any token request that uses the `POP` authentication scheme.
...
let shrNonce = ""; // Globally scoped variable
...
const popTokenRequest = {
    ...
    resourceRequestMethod: "POST",
    resourceRequestUri: "YOUR_RESOURCE_ENDPOINT"
    shrNonce: shrNonce // SHR Nonce is invalid as an empty string at this point
};
...
return fetch(resourceEndpointData.endpoint, options)
    ...
    .catch(error => {
        // If error is 401 Unauthorized, contains WWW-Authenticate and error code is nonce_missing or nonce_expired
            lastResponseHeaders = response.headers;
            const authHeaderParser = new AuthenticationHeaderParser(response.headers);
            shrNonce = authHeaderParser.getShrNonce(); // Empty string is replaced with valid nonce from WWW-Authenticate header
        // else -> Other errors that don't merit a retry
            // Deal with other errors as desired
});
```
...
const async callPopResource(
    ...
    return fetch(resourceEndpointData.endpoint, options)
        ...
        .then(response => {
            // 200 OK if nonce was valid
            lastResponseHeaders = response.headers; // Authentication-Info header contains nextnonce
            return { response.body, lastResponeHeaders }
        })
        .catch(error => {
            // If error is 401 Unauthorized, contains WWW-Authenticate and error code is nonce_missing or nonce_expired
                lastResponseHeaders = response.headers;
                return callPopResource(publicClientApplication, resourceEndpointData, lastResponseHeaders);
            // else -> Other errors that don't merit a retry
                // Deal with other errors as desired
    });
}
