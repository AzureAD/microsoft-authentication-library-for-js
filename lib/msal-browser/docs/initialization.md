...
-   **Cross-origin iframe scenarios:** If your app runs inside a cross-origin iframe, NAA is the **recommended** approach for authentication. In browsers that enforce [third-party storage partitioning](https://developers.google.com/privacy-sandbox/cookies/storage-partitioning) (Chromium 115+, with other browsers expected to follow), popup flows (`loginPopup`, `acquireTokenPopup`) and flows that use a hidden iframe (`ssoSilent`, `acquireTokenSilent` when falling back to iframe-based renewal) will fail because the [redirect bridge's](./redirect-bridge.md) `BroadcastChannel` is partitioned by top-level site. NAA avoids this limitation entirely by delegating authentication to the host application at the top level. See [Redirect Bridge — Cross-Origin Iframe Limitation](./redirect-bridge.md#cross-origin-iframe-limitation) and [Using MSAL in iframed apps](./iframe-usage.md) for more details.
