...
import { Constants, PersistentCacheKeys, StringUtils, CommonAuthorizationCodeRequest, ICrypto, AccountEntity, IdTokenEntity, AccessTokenEntity, RefreshTokenEntity, AppMetadataEntity, CacheManager, ServerTelemetryEntity, ThrottlingEntity, ProtocolUtils, Logger, AuthorityMetadataEntity, DEFAULT_CRYPTO_IMPLEMENTATION, AccountInfo, CcsCredential, CcsCredentialType, IdToken, RequestThumbprint } from "@azure/msal-common";
...
export class BrowserCacheManager extends CacheManager {
    ...
    generateBrokerResponseKey(responseThumbprint: string): string {
        return `${InMemoryCacheKeys.BROKER_RESPONSE}.${responseThumbprint}`;
    }
    ...
    getBrokerResponseByOrigin(messageOrigin: string): string | null {
        ...
        const embeddedAppKey = `${InMemoryCacheKeys.BROKER_RESPONSE}.${messageOrigin}`;
    ...
    getBrokerResponseByThumbprint(reqThumbprint: RequestThumbprint, embeddedAppOrigin: string): string | null {
        ...
        const responseCacheKey = `${InMemoryCacheKeys.BROKER_RESPONSE}.${responseThumbprint}`;
