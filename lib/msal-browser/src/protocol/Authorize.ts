...
import { createNewGuid, decryptEarResponse } from "../crypto/BrowserCrypto.js";
import { IPlatformAuthHandler } from "../broker/nativeBroker/IPlatformAuthHandler.js";
import * as BrowserUtils from "../utils/BrowserUtils.js";
import { initializeAuthorizationRequest } from "../interaction_client/StandardInteractionClient.js";
...
/**
 * Request passed to {@link getAuthCodeUrl}. A {@link CommonAuthorizationUrlRequest}
 * where the only required field is the caller-generated PKCE `codeChallenge`
 * (MSAL cannot synthesize it — the caller keeps the matching verifier).
 * Everything else is optional and resolved from config (e.g. `redirectUri` and
 * `responseMode` from `config.auth`) or generated/defaulted by MSAL
 * (e.g. `scopes` default to the OIDC scopes, `nonce` and `correlationId` are
 * generated).
 */
export type AuthCodeUrlRequest = Partial<CommonAuthorizationUrlRequest> & {
    /** PKCE code challenge (S256) generated by the caller, who keeps the verifier. */
    codeChallenge: string;
};

/**
 * Builds a fully-formed /authorize URL for an authorization-code + PKCE flow
 *
 * @param config - {@link Configuration} MSAL configuration.
 * @param request - {@link AuthCodeUrlRequest}. The caller supplies the PKCE
 * `codeChallenge` and keeps the matching verifier.
 * @param performanceClient - {@link IPerformanceClient} Optional performance client.
 * @returns The fully-formed /authorize URL.
 */
export async function getAuthCodeUrl(
    ...
    const authority = await AuthorityFactory.createDiscoveredInstance(
        Authority.generateAuthority(authorityString, request.azureCloudOptions),
    ...
    const baseRequest = await initializeAuthorizationRequest(
        request,
    ...
    const authCodeUrlRequest: CommonAuthorizationUrlRequest = {
        ...
        /*
         * Round-trip the caller's state verbatim (un-wrapped) so it owns
         * response correlation
         */
        state: request.state ?? baseRequest.state,
    };
