 ...
 * next step - the correlation id, the continuation token to present, the scenario being driven,
 * and the next-step hrefs. The scenario is injected by the flow-specific controller (it is the
 * generic client's flow identity) and re-emitted so downstream results/errors can be tagged with
 * the originating flow. Public L1 states carry this back into the client on the next action; it is
