...
export function validateCryptoAvailable(
    ...
    if (!skipValidateSubtleCrypto && !self.crypto.subtle) {
        throw createBrowserAuthError(
...
export function getRandomValues(dataBuffer: Uint8Array): Uint8Array {
    return self.crypto.getRandomValues(dataBuffer);
}
...
function getRandomUint32(): number {
    self.crypto.getRandomValues(UINT32_ARR);
    return UINT32_ARR[0];
}
...
export async function importEarKey(earJwk: string): Promise<CryptoKey> {
    ...
    return self.crypto.subtle.importKey(RAW, keyBuffer, AES_GCM, false, [
        DECRYPT,
    ]);
}
...
export async function generateBaseKey(): Promise<ArrayBuffer> {
    ...
    return self.crypto.subtle.exportKey(RAW, key);
}
...
export async function generateHKDF(baseKey: ArrayBuffer): Promise<CryptoKey> {
    return self.crypto.subtle.importKey(RAW, baseKey, HKDF, false, [
        DERIVE_KEY,
    ]);
}
...
export async function encrypt(
    ...
    const nonce = self.crypto.getRandomValues(new Uint8Array(16));
