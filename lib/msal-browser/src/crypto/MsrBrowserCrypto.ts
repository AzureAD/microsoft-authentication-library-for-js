...
export class MsrBrowserCrypto implements ISubtleCrypto {
    ...
    async generateKey(
        ...
        return window.msrCrypto.subtle.generateKey(
            algorithm,
            extractable,
            keyUsages
    ...
    async exportKey(key: CryptoKey): Promise<JsonWebKey> {
        return window.msrCrypto.subtle.exportKey(
            KEY_FORMAT_JWK,
            key
        ) as Promise<JsonWebKey> as Promise<JsonWebKey>;
    }
    ...
    async importKey(
        ...
        return window.msrCrypto.subtle.importKey(
            KEY_FORMAT_JWK,
            keyData,
            algorithm,
            extractable,
    ...
    async sign(
        ...
        return window.msrCrypto.subtle.sign(
            algorithm,
            key,
            data
    ...
    async digest(
        ...
        return window.msrCrypto.subtle.digest(
            algorithm,
            data
        ) as Promise<ArrayBuffer>;
    }
