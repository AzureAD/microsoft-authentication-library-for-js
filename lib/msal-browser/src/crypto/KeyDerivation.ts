...
export class KeyDerivation {
     ...
     * @param label 
     */
    public async computeKDFInCounterMode(derivationKey: CryptoKey, inputData: string | Uint8Array, label: string): Promise<ArrayBuffer> {
        const fixedInput = this.generateFixedInput(inputData, label);
    ...
    /**
     * Encodes the derivation context (inputData) and label strings into byte arrays and returns
     ...
     * Returned binary string is in the form Label || 0x00 || inputData || [L]_2
     * @param inputData
     * @param label 
     */
    private generateFixedInput(inputData: string | Uint8Array, label: string): Uint8Array {
        // Encode context if not already a byte array
        const inputDataBytes = (typeof inputData === "string") ?
            Uint8Array.from(window.atob(inputData), (v) => v.charCodeAt(0)) :
            inputData;
        ...
        const fixedInput = new Uint8Array(labelBytes.length + zeroOctet.length + inputDataBytes.length + keyLengthBytes.length);

        // Concatenate [Label || 0x00 || inputData || [L]_2]
        fixedInput.set(labelBytes, 0); // Label
        fixedInput.set([0], labelBytes.length); // 0x00
        fixedInput.set(inputDataBytes, labelBytes.length + zeroOctet.length); // ctx or SHA-256(ctx || payload)
