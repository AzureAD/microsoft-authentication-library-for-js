...
describe("Base64Decode.ts Unit Tests", () => {
    ...
    describe("decode", () => {
        ...
        it("MSAL Test Vectors", () => {
            ...
            // Client Info B64
            expect(
                b64Decode.decode(TEST_DATA_CLIENT_INFO.TEST_UID_ENCODED)
            ).toBe(TEST_DATA_CLIENT_INFO.TEST_UID);
            expect(
                b64Decode.decode(TEST_DATA_CLIENT_INFO.TEST_UTID_ENCODED)
            ).toBe(TEST_DATA_CLIENT_INFO.TEST_UTID);
            expect(
                b64Decode.decode(TEST_DATA_CLIENT_INFO.TEST_RAW_CLIENT_INFO)
            ...
            expect(b64Decode.decode(b64Encode.encode(stringifiedClaims))).toBe(
                stringifiedClaims
            );
            ...
            expect(b64Decode.decode(b64Encode.encode(stringifiedReq))).toBe(
                stringifiedReq
            );
        });
