import { decryptEarResponse, generateEarKey } from "../../src/crypto/BrowserCrypto.js";
...
import { generateValidEarJWE, TEST_TOKEN_RESPONSE, validEarJWE, validEarJWK } from "../utils/StringConstants.js";
...
describe("BrowserCrypto Tests", () => {
    describe("generateEarKey", () => {
        it("Returns Base64 encoded string of ear_jwk", async () => {
            ...
            const genKeySpy = jest.spyOn(window.crypto.subtle, "generateKey").mockResolvedValue(key);
            const exportKeySpy = jest.spyOn(window.crypto.subtle, "exportKey").mockResolvedValue(rawKey);
            const encodedJwk = await generateEarKey();
            expect(genKeySpy).toHaveBeenCalledWith({name: "AES-GCM", length: 256}, true, ["encrypt", "decrypt"]);
    ...
    describe("decryptEarResponse", () => {
        it("Throws if ear_jwe has fewer than 5 parts", (done) => {
            decryptEarResponse(validEarJWK, "header.iv.ciphertext.tag").catch((e) => {
                expect(e).toBeInstanceOf(BrowserAuthError);
                expect(e.errorCode).toBe(BrowserAuthErrorCodes.failedToDecryptEarResponse);
        ...
        it("Throws if earJwk does not have a 'k' property", (done) => {
            const encodedJwk = base64Encode(JSON.stringify({alg: "dir", kty: "oct", key: "testKey"}));
        ...
        it("Throws if earJwk cannot be B64 decoded", (done) => {
            decryptEarResponse(JSON.stringify({alg: "dir", kty: "oct", k: "testKey"}), validEarJWE).catch((e) => {
                expect(e).toBeInstanceOf(BrowserAuthError);
                expect(e.errorCode).toBe(BrowserAuthErrorCodes.failedToDecryptEarResponse);
                expect(e.subError).toBe("import_key");
                done();
        ...
        it("Throws if earJwk 'k' property is not a raw encryption key", (done) => {
            decryptEarResponse(base64Encode(JSON.stringify({alg: "dir", kty: "oct", k: "testKey"})), validEarJWE).catch((e) => {
                expect(e).toBeInstanceOf(BrowserAuthError);
                expect(e.errorCode).toBe(BrowserAuthErrorCodes.failedToDecryptEarResponse);
                expect(e.subError).toBe("import_key");
                done();
        ...
        it("Successfully decrypts ear_jwe with given earJwk", async () => {
            const jwe = await generateValidEarJWE(JSON.stringify(TEST_TOKEN_RESPONSE.body), validEarJWK);
            const decryptedString = await decryptEarResponse(validEarJWK, jwe);
            expect(JSON.parse(decryptedString)).toEqual(TEST_TOKEN_RESPONSE.body);
        });
    });
});