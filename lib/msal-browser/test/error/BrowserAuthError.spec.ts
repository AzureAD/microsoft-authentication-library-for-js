...
describe("BrowserAuthError Unit Tests", () => {
    ...
    it("createNonBrowserEnvironmentError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createNonBrowserEnvironmentError();
    ...
    it("createPkceNotGeneratedError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createPkceNotGeneratedError("PKCE Error detail.");
        ...
        expect(
            err.message?.includes(BrowserAuthErrorMessage.pkceNotGenerated.desc)
        ).toBe(true);
        expect(err.name).toBe("BrowserAuthError");
        expect(err.stack?.includes("BrowserAuthError.spec.ts")).toBe(true);
    });
    ...
    it("createCryptoNotAvailableError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createCryptoNotAvailableError(
        ...
        expect(
            err.errorMessage?.includes(
                BrowserAuthErrorMessage.cryptoDoesNotExist.desc
            )
        ).toBe(true);
        expect(
            err.message?.includes(
    ...
    it("createEmptyHashError()", () => {
        ...
        expect(
            err.message?.includes(BrowserAuthErrorMessage.hashEmptyError.desc)
        ).toBe(true);
        expect(err.name).toBe("BrowserAuthError");
        expect(err.stack?.includes("BrowserAuthError.spec.ts")).toBe(true);
    });
    ...
    it("createPopupWindowError()", () => {
        ...
        expect(
            err.errorMessage?.includes(
                BrowserAuthErrorMessage.popupWindowError.desc
            )
        ).toBe(true);
        expect(
            err.message?.includes(BrowserAuthErrorMessage.popupWindowError.desc)
        ).toBe(true);
        expect(err.name).toBe("BrowserAuthError");
        expect(err.stack?.includes("BrowserAuthError.spec.ts")).toBe(true);
    });
    ...
    it("createPopupWindowError() with error message", () => {
        ...
        const err: BrowserAuthError =
            BrowserAuthError.createPopupWindowError(testErrMessage);
    ...
    it("createEmptyWindowCreatedError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createEmptyWindowCreatedError();
        ...
        expect(
            err.message?.includes(BrowserAuthErrorMessage.emptyWindowError.desc)
        ).toBe(true);
        expect(err.name).toBe("BrowserAuthError");
        expect(err.stack?.includes("BrowserAuthError.spec.ts")).toBe(true);
    });
    ...
    it("createMonitorPopupTimeoutError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createMonitorPopupTimeoutError();
    ...
    it("createRedirectInIframeError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createRedirectInIframeError(false);
    ...
    it("createBlockReloadInHiddenIframeError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createBlockReloadInHiddenIframeError();
        ...
        expect(
            err.errorMessage.includes(
                BrowserAuthErrorMessage.blockTokenRequestsInHiddenIframeError
                    .desc
            )
        ).toBe(true);
        expect(
    ...
    it("createIframeClosedPrematurelyError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createIframeClosedPrematurelyError();
    ...
    it("createSilentPromptValueError()", () => {
        ...
        const err: BrowserAuthError =
            BrowserAuthError.createSilentPromptValueError(promptVal);
    ...
    it("createInvalidCacheTypeError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createInvalidCacheTypeError();
        ...
        expect(
            err.message?.includes(BrowserAuthErrorMessage.invalidCacheType.desc)
        ).toBe(true);
        expect(err.name).toBe("BrowserAuthError");
        expect(err.stack?.includes("BrowserAuthError.spec.ts")).toBe(true);
    });
    ...
    it("createUnableToLoadTokenError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createUnableToLoadTokenError(
                "Load Token Error Detail"
            );
    ...
    it("createSigningKeyNotFoundInStorageError()", () => {
        ...
        expect(
            err.errorMessage?.includes(
                BrowserAuthErrorMessage.signingKeyNotFoundInStorage.desc
            )
        ).toBe(true);
        expect(
            err.message?.includes(
    ...
    it("createDatabaseUnavailableError()", () => {
        const err: BrowserAuthError =
            BrowserAuthError.createDatabaseUnavailableError();
