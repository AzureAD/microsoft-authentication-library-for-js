...
describe("BrowserConfigurationAuthError Unit Tests", () => {
    it("BrowserConfigurationAuthError object can be created", () => {
        ...
        const err: BrowserConfigurationAuthError =
            new BrowserConfigurationAuthError(TEST_ERROR_CODE, TEST_ERROR_MSG);
    ...
    it("createStorageNotSupportedError()", () => {
        const err: BrowserConfigurationAuthError =
            BrowserConfigurationAuthError.createStorageNotSupportedError(
                "notAStorageLocation"
            );
        ...
        expect(
            err.errorMessage.includes(
                BrowserConfigurationAuthErrorMessage.storageNotSupportedError
                    .desc
            )
        ).toBe(true);
        expect(
