...
describe("PublicClientApplication.ts Class Unit Tests", () => {
    ...
    describe("ssoSilent() Tests", () => {
        ...
        it("successfully returns a token response (account + empty cache)", async () => {
            const testServerTokenResponse = {
                token_type: TEST_CONFIG.TOKEN_TYPE_BEARER,
                scope: TEST_CONFIG.DEFAULT_SCOPES.join(" "),
                expires_in: TEST_TOKEN_LIFETIMES.DEFAULT_EXPIRES_IN,
            ...
            sinon.stub(PublicClientApplication.prototype, "getAllAccounts").returns([ testAccount ]);
            sinon.stub(AuthorizationCodeClient.prototype, "getAuthCodeUrl").resolves(testNavUrl);
            const loadFrameSyncSpy = sinon.spy(SilentHandler.prototype, <any>"loadFrameSync");
            sinon.stub(SilentHandler.prototype, "monitorIframeForHash").resolves(TEST_HASHES.TEST_SUCCESS_CODE_HASH_SILENT);
            sinon.stub(SilentHandler.prototype, "handleCodeResponse").resolves(testTokenResponse);
            ...
            sinon.stub(CryptoOps.prototype, "createNewGuid").returns(RANDOM_TEST_GUID);
            sinon.stub(PublicClientApplication.prototype, "acquireTokenSilent").rejects(ClientAuthError.createNoTokensFoundError());
            const tokenResp = await pca.ssoSilent({
                redirectUri: TEST_URIS.TEST_REDIR_URI,
                scopes: TEST_CONFIG.DEFAULT_SCOPES,
                account: testAccount
        ...
        it("successfully returns a token response (account + skipCache)", async () => {
            ...
            sinon.stub(CryptoOps.prototype, "createNewGuid").returns(RANDOM_TEST_GUID);
            sinon.stub(PublicClientApplication.prototype, "acquireTokenSilent").rejects(new Error("acquireTokenSilent should not be called"));
            const tokenResp = await pca.ssoSilent({
                redirectUri: TEST_URIS.TEST_REDIR_URI,
                scopes: TEST_CONFIG.DEFAULT_SCOPES,
                account: testAccount,
        ...
        it("successfully returns a cached token response (sid)", async () => {
            const testServerTokenResponse = {
                token_type: TEST_CONFIG.TOKEN_TYPE_BEARER,
                scope: TEST_CONFIG.DEFAULT_SCOPES.join(" "),
                expires_in: TEST_TOKEN_LIFETIMES.DEFAULT_EXPIRES_IN,
            ...
            sinon.stub(PublicClientApplication.prototype, "getAllAccounts").returns([ testAccount ]);
            sinon.stub(PublicClientApplication.prototype, "acquireTokenSilent").resolves(testTokenResponse);
            const tokenResp = await pca.ssoSilent({
                redirectUri: TEST_URIS.TEST_REDIR_URI,
                scopes: TEST_CONFIG.DEFAULT_SCOPES,
                sid: TEST_CONFIG.SID,
        ...
        it("successfully returns a cached token response (login_hint)", async () => {
            const testServerTokenResponse = {
                token_type: TEST_CONFIG.TOKEN_TYPE_BEARER,
                scope: TEST_CONFIG.DEFAULT_SCOPES.join(" "),
                expires_in: TEST_TOKEN_LIFETIMES.DEFAULT_EXPIRES_IN,
        ...
        it("successfully returns a cached token response (account passed)", async () => {
            const testServerTokenResponse = {
                token_type: TEST_CONFIG.TOKEN_TYPE_BEARER,
                scope: TEST_CONFIG.DEFAULT_SCOPES.join(" "),
                expires_in: TEST_TOKEN_LIFETIMES.DEFAULT_EXPIRES_IN,
    ...
    describe("getAccount tests", () => {
        ...
        const testAccountInfo3: AccountInfo = {
            ...
            environment: "login.windows.net",
            tenantId: TEST_DATA_CLIENT_INFO.TEST_UTID,
            username: "anotherone@microsoft.com",
            name: "Abe Lincoln",
            localAccountId: TEST_CONFIG.OID,
            idTokenClaims: {
                sid: TEST_CONFIG.SID
        ...
        it("getAccountBySessionId returns account specified", () => {
            const allAccountStub = sinon.stub(PublicClientApplication.prototype, "getAllAccounts").returns([ testAccountInfo3 ]);
            const account = pca.getAccountBySessionId(TEST_CONFIG.SID);
            expect(account).to.deep.equal(testAccountInfo3);
            allAccountStub.restore();
        });

        it("getAccountBySessionId returns null if null sid is passed", () => {
            const account = pca.getAccountBySessionId(null);
            expect(account).to.be.null;
        });
    });
