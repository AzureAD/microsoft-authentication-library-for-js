...
=======
        describe("initialize tests", () => {
            globalThis.MessageChannel =
                require("worker_threads").MessageChannel; // jsdom does not include an implementation for MessageChannel
>>>>>>> Stashed changes
...
>>>>>>> Stashed changes
        ...
        describe("handleRedirectPromise", () => {
            ...
            it("Multiple concurrent calls to handleRedirectPromise return the same promise", async () => {
                ...
                const testTokenResponse: AuthenticationResult = {
                    ...
                    expiresOn: new Date(
                        Date.now() +
                            testServerTokenResponse.body.expires_in * 1000
                    ),
                    account: testAccount,
                    tokenType: AuthenticationScheme.BEARER,
                };
                ...
                expect(tokenResponse1.uniqueId).toEqual(
                    testTokenResponse.uniqueId
                );
                expect(tokenResponse1.tenantId).toEqual(
                    testTokenResponse.tenantId
                );
                expect(tokenResponse1.scopes).toEqual(testTokenResponse.scopes);
                expect(tokenResponse1.idToken).toEqual(
                    testTokenResponse.idToken
                );
                ...
                expect(tokenResponse2.uniqueId).toEqual(
                    testTokenResponse.uniqueId
                );
                expect(tokenResponse2.tenantId).toEqual(
                    testTokenResponse.tenantId
                );
                expect(tokenResponse2.scopes).toEqual(testTokenResponse.scopes);
                expect(tokenResponse2.idToken).toEqual(
                    testTokenResponse.idToken
                );
        ...
        describe("acquireTokenRedirect", () => {
            ...
            it("throws if interaction is currently in progress for a different clientId", async () => {
                ...
                expect(browserStorage.isInteractionInProgress(true)).toBe(
                    false
                );
                expect(browserStorage.isInteractionInProgress(false)).toBe(
                    true
                );
                expect(
                    secondInstanceStorage.isInteractionInProgress(true)
                ).toBe(true);
                expect(
                    secondInstanceStorage.isInteractionInProgress(false)
                ).toBe(true);
                await expect(
                    pca.acquireTokenRedirect({ scopes: ["openid"] })
                ).rejects.toMatchObject(
            ...
            it("throws error if called in a popup", (done) => {
                ...
                pca.acquireTokenRedirect({ scopes: ["openid"] })
                    .catch((e) => {
                        ...
                        expect(e.errorCode).toEqual(
                            BrowserAuthErrorMessage
                                .blockAcquireTokenInPopupsError.code
                        );
                        expect(e.errorMessage).toEqual(
                            BrowserAuthErrorMessage
                                .blockAcquireTokenInPopupsError.desc
                        );
                        done();
                    })
                    .finally(() => {
        ...
        describe("acquireTokenSilent", () => {
            ...
            it("makes one network request with multiple parallel silent requests with same request", async () => {
                ...
                expect(
                    silentATStub.calledWith(expectedTokenRequest)
                ).toBeTruthy();
                expect(atsSpy.calledOnce).toBe(true);
                expect(silentATStub.callCount).toEqual(1);
                expect(parallelResponse[0]).toEqual(testTokenResponse);
            ...
            it("makes network requests for each distinct request when acquireTokenSilent is called in parallel", async () => {
                ...
                const silentRequest2 = pca.acquireTokenSilent(tokenRequest1);
                const silentRequest3 = pca.acquireTokenSilent(tokenRequest2);
                const popSilentRequest1 =
                    pca.acquireTokenSilent(popTokenRequest1);
                const popSilentRequest2 =
                    pca.acquireTokenSilent(popTokenRequest1);
                const popSilentRequest3 =
                    pca.acquireTokenSilent(popTokenRequest2);
                const sshCertSilentRequest1 =
                    pca.acquireTokenSilent(sshCertRequest1);
                const sshCertSilentRequest2 =
                    pca.acquireTokenSilent(sshCertRequest1);
                ...
                expect(
                    silentATStub.calledWith(expectedTokenRequest1)
                ).toBeTruthy();
                expect(
                    silentATStub.calledWith(expectedTokenRequest2)
                ).toBeTruthy();
                expect(
                    silentATStub.calledWith(expectedPopTokenRequest1)
                ).toBeTruthy();
            ...
            it("makes network requests for each distinct request including claims when acquireTokenSilent is called in parallel with claimsBasedCaching is enabled", async () => {
                ...
                const sshCertSilentRequest3 =
                    pca.acquireTokenSilent(sshCertRequest2);
                const claimsSilentRequest1 =
                    pca.acquireTokenSilent(claimsRequest1);
                const claimsSilentRequest2 =
                    pca.acquireTokenSilent(claimsRequest1);
                const claimsSilentRequest3 =
                    pca.acquireTokenSilent(claimsRequest2);
                await Promise.all([
                    silentRequest1,
                    silentRequest2,
                    silentRequest3,
        ...
        describe("Event API tests", () => {
            it("can add an event callback", (done) => {
                const subscriber = (message: EventMessage) => {
                    ...
                    expect(message.interactionType).toEqual(
        ...
        describe("Logger", () => {
            ...
            test("logger undefined", async () => {
                ...
                expect(authApp.getLogger().info("Test logger")).toEqual(
                    undefined
                );
            });
        });
        ...
        describe("preflightBrowserEnvironmentCheck", () => {
            ...
            it("calls setInteractionInProgress", () => {
                ...
                // @ts-ignore
                expect(pca.browserStorage.getInteractionInProgress())
                    .toBeTruthy;
            });

            it("doesnt call setInteractionInProgress", () => {
                // @ts-ignore
                pca.preflightBrowserEnvironmentCheck(
                    InteractionType.Popup,
                    false
                );
        ...
        describe("hydrateCache tests", () => {
            ...
            it("hydrates cache with the provided id and access tokens", async () => {
                ...
                expect(result.idToken).toEqual(
                    testAuthenticationResult.idToken
                );
    ...
    describe("Multi-tenant mode", () => {
        ...
        describe("getAllAccounts", () => {
            // Home Account
            const homeAccount: AccountEntity =
                AccountEntity.createFromAccountInfo(TEST_HOME_ACCOUNT_INFO);

            const homeAccountIdToken = {
                realm: homeAccount.realm,
                environment: homeAccount.environment,
                credentialType: CredentialType.ID_TOKEN,
                secret: TEST_TOKENS.IDTOKEN_V2,
                clientId: TEST_CONFIG.MSAL_CLIENT_ID,
                homeAccountId: homeAccount.homeAccountId,
            };

            const guestTenantAccount: AccountEntity =
                AccountEntity.createFromAccountInfo(TEST_GUEST_ACCOUNT_INFO);

            const guestTenantAccountIdToken = {
                realm: guestTenantAccount.realm,
                environment: guestTenantAccount.environment,
                CredentialType: CredentialType.ID_TOKEN,
                secret: TEST_TOKENS.GUEST_ID_TOKEN_V2,
                homeAccountId: guestTenantAccount.homeAccountId,
            };
            ...
            beforeEach(async () => {
                ...
                // @ts-ignore
                pca.getBrowserStorage().setAccount(homeAccount);
                // @ts-ignore
                pca.getBrowserStorage().setIdTokenCredential(
                    homeAccountIdToken
                );
                // @ts-ignore
                pca.getBrowserStorage().setIdTokenCredential(
                    guestTenantAccountIdToken
                );
            });
            ...
            it("getAllAccounts with no filter returns all signed in accounts with a tenants list", () => {
                const accounts = pca.getAllAccounts();
                expect(accounts).toHaveLength(1);
                expect(accounts[0].idToken).not.toBeUndefined();
                expect(accounts[0].tenants).not.toBeUndefined();
            });
        });
    });
});
