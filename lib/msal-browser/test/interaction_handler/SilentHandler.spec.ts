...
import { PkceCodes, AuthorityFactory, CommonAuthorizationCodeRequest, Constants, AuthorizationCodeClient, ProtocolMode, Logger, LoggerOptions, AuthenticationScheme, AuthorityOptions, ClientConfiguration, IPerformanceClient } from "@azure/msal-common";
...
describe("SilentHandler.ts Unit Tests", () => {
    ...
    beforeEach(() => {
        ...
        authConfig.storageInterface = new TestStorageManager(TEST_CONFIG.MSAL_CLIENT_ID, authConfig.cryptoInterface!, logger);
    ...
    describe("monitorIframeForHash", () => {
        ...
        it("Throws hash empty error", done => {
            ...
            silentHandler.monitorIframeForHash(iframe, 1000)
                .catch((e) => {
                    expect(e).toBeInstanceOf(BrowserAuthError);
                    expect(e).toMatchObject(BrowserAuthError.createEmptyHashError());
                    done();
                });
        ...
        it("Throws hashDoesNotContainKnownProperties error", done => {
            ...
            silentHandler.monitorIframeForHash(iframe, 1000)
                .catch((e) => {
                    expect(e).toBeInstanceOf(BrowserAuthError);
                    expect(e).toMatchObject(BrowserAuthError.createHashDoesNotContainKnownPropertiesError());
                    done();
                });
