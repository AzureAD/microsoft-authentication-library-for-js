...
export const testNavUrl = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?response_type=code&client_id=${encodeURIComponent(`${TEST_CONFIG.MSAL_CLIENT_ID}`)}&scope=user.read%20openid%20profile&redirect_uri=https%3A%2F%2Flocalhost%3A8081%2Findex.html&client-request-id=${encodeURIComponent(`${RANDOM_TEST_GUID}`)}&code_challenge=JsjesZmxJwehdhNY9kvyr0QOeSMEvryY_EHZo3BKrqg&code_challenge_method=S256&state=${encodeURIComponent(`${TEST_STATE_VALUES.TEST_STATE}`)}&nonce=${encodeURIComponent(`${RANDOM_TEST_GUID}`)}&client_info=1&x-client-SKU=msal.js.browser&x-client-VER=2.0.0-beta.2&x-client-OS=&x-client-CPU=&response_mode=fragment`;
