...
export const TEST_URIS = {
    ...
    TEST_REDIR_WITH_PERCENTENCODED_SYMBOLS_URI:
        "https://localhost:8081/%D1%80/index.html",
...
export const TEST_DATA_CLIENT_INFO = {
    ...
    TEST_UID_ENCODED: "MDAwMDAwMDAtMDAwMC0wMDAwLTY2ZjMtMzMzMmVjYTdlYTgx",
    TEST_UTID: "3338040d-6c67-4c5b-b112-36a304b66dad",
    TEST_UTID_ENCODED: "MzMzODA0MGQtNmM2Ny00YzViLWIxMTItMzZhMzA0YjY2ZGFk",
...
)}&scope=user.read%20openid%20profile%20offline_access&redirect_uri=https%3A%2F%2Flocalhost%3A8081%2Findex.html&client-request-id=${encodeURIComponent(
    `${RANDOM_TEST_GUID}`
)}&response_mode=fragment&response_type=code&x-client-SKU=msal.js.browser&x-client-VER=${version}&x-app-name=${
    TEST_CONFIG.applicationName
}&x-app-ver=${
    TEST_CONFIG.applicationVersion
}&client_info=1&code_challenge=JsjesZmxJwehdhNY9kvyr0QOeSMEvryY_EHZo3BKrqg&code_challenge_method=S256&nonce=${encodeURIComponent(
    `${RANDOM_TEST_GUID}`
...
export function getTestAuthenticationResult(): AuthenticationResult {
    /*
     * Read the clock once. Calling nowSeconds() separately for expiresOn and
     * extExpiresOn lets a second boundary fall between the two reads, producing
     * a single result object whose two timestamps disagree by a second.
     */
    const expiresOnSeconds =
    ...
    return {
        ...
        idTokenClaims: ID_TOKEN_CLAIMS,
        fromCache: false,
        expiresOn: new Date(expiresOnSeconds * 1000),
        tokenType: "Bearer",
        ...
        cloudGraphHostName: "",
        code: undefined,
        extExpiresOn: new Date(expiresOnSeconds * 1000),
...
/**
 * Asserts that an AuthenticationResult produced by a flow matches the expected
 * result, tolerating a small difference in the expiry timestamps.
 *
 * expiresOn/extExpiresOn are derived from a clock read taken inside the flow
 * under test, so they can land in a later second than the expectation the test
 * builds. Comparing them exactly makes the test fail intermittently whenever a
 * second boundary falls between the two reads.
 *
 * Use this only when the actual result is generated by the code under test. If
 * the expected result was handed to a mock, reuse that same object instead so
 * the comparison stays exact.
 *
 * The tolerance is deliberately generous relative to how long a flow can take
 * on a loaded CI machine, but it stays tiny next to the token lifetimes these
 * tests use (an hour), so a genuinely wrong expiry -- wrong base time, wrong
 * units, or a missing offset -- is still caught.
 */
export function expectAuthenticationResult(
    actual: AuthenticationResult | null,
    expected: AuthenticationResult,
    toleranceMs: number = 30000
): void {
    expect(actual).not.toBeNull();

    // Compare every field except the two clock-derived expiry timestamps.
    const {
        expiresOn: actualExpiresOn,
        extExpiresOn: actualExtExpiresOn,
        ...actualRest
    } = actual as AuthenticationResult;
    const {
        expiresOn: expectedExpiresOn,
        extExpiresOn: expectedExtExpiresOn,
        ...expectedRest
    } = expected;
