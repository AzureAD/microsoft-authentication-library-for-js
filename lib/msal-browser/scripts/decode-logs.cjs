...
function loadSingleMapping(mappingPath) {
    ...
    try {
        ...
        let packageVariant = "";
        if (mappingPath.includes("msal-browser-1p")) {
            packageName = "@azure/msal-browser-1p";
        } else if (mappingPath.includes("msal-browser") && !mappingPath.includes("msal-browser-1p")) {
            packageName = "@azure/msal-browser";
            if (mappingPath.includes("custom-auth-path")) {
                packageVariant = " (custom-auth-path)";
            }
        } else if (mappingPath.includes("msal-common")) {
    ...
    if (metadata?.sources && metadata.sources.length > 0) {
        if (metadata.sources.length === 1) {
            ...
            metadata.sources.forEach(source => {
                const fileName = source.packageName && source.packageName !== "unknown"
                    ? `${source.packageName}${source.packageVariant || ""}`
