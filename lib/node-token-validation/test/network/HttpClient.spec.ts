...
describe("HttpClient", () => {
    ...
    describe("sendGetRequestAsync", () => {
        ...
        it("returns network error if response times out", (done) => {
            ...
            httpClient.sendGetRequestAsync("url").catch((e) => {
                expect(e).toBeInstanceOf(ClientAuthError);
                expect(e.errorCode).toBe(
                    ClientAuthErrorMessage.networkError.code
                );
                expect(
                    e.errorMessage.includes(HttpStatusCode.RequestTimeout)
                ).toBe(true);
        ...
        it("returns network error if service unavailable", (done) => {
            ...
            httpClient.sendGetRequestAsync("url").catch((e) => {
                ...
                expect(
                    e.errorMessage.includes(HttpStatusCode.ServiceUnavailable)
                ).toBe(true);
                expect(e.errorMessage.includes("url")).toBe(true);
                done();
            });
            ...
            httpClient.sendGetRequestAsync("url").catch((e) => {
                ...
                expect(
                    e.errorMessage.includes(
                        "No server response for get request"
                    )
                ).toBe(true);
                expect(e.errorMessage.includes("url")).toBe(true);
                done();
