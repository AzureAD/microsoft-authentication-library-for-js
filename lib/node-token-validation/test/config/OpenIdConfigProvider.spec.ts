...
describe("OpenIdConfigProvider", () => {
    ...
    describe("constructor", () => {
        it("exports a class", () => {
            const cache = new NodeCacheManager(
                logger,
                TEST_CONSTANTS.CLIENT_ID,
                DEFAULT_CRYPTO_IMPLEMENTATION
            );
            const provider = new OpenIdConfigProvider(
                config,
    ...
    describe("fetchJwksUriFromEndpoint", () => {
        it("returns jwks_uri", async () => {
            ...
            const mockDiscoveredInstance = jest
                .spyOn(AuthorityFactory, "createDiscoveredInstance")
                .mockReturnValue(Promise.resolve(mockAuthority));
        ...
        it("throws error if openIdResponse does not contain jwks_uri", async () => {
            ...
            await provider.fetchJwksUriFromEndpoint().catch((e) => {
                expect(mockDiscoveredInstance).toHaveBeenCalledTimes(1);
