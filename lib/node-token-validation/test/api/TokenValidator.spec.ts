...
describe("TokenValidator", () => {
    ...
    describe("validateTokenMiddleware", () => {
        it("adds access token to request header if resource passed in and access token exists on session", (done) => {
            ...
            const validateTokenSpy = jest
                .spyOn(validator, "validateTokenFromRequest")
                .mockReturnValue(Promise.resolve(validateTokenResponse));
            ...
            const middlewareFunction = validator.validateTokenMiddleware(
                defaultOptions,
                "myApi"
            );
    ...
    describe("validateTokenFromRequest", () => {
        it("validates token from request authorization header", async () => {
            ...
            const validateTokenSpy = jest
                .spyOn(validator, "validateToken")
                .mockReturnValue(Promise.resolve(validateTokenResponse));

            const result = await validator.validateTokenFromRequest(
                request,
                defaultOptions
            );
            ...
            expect(validateTokenSpy).toReturnWith(
                Promise.resolve(validateTokenResponse)
            );
        });
        ...
        it("throws error if authorization header in request is not bearer, and no token in body", async () => {
            ...
            await validator
                .validateTokenFromRequest(request, defaultOptions)
                .catch((e) => {
    ...
    describe("validateTokenFromResponse", () => {
        it("validates id token if present on msal response and options provided", async () => {
            ...
            const result = await validator.validateTokenFromResponse(
                msalResponse,
                defaultOptions
            );
        ...
        it("validates id token with accessToken if accessToken returned in response", async () => {
            ...
            expect(validateTokenSpy).toHaveBeenCalledWith(
                "id-token",
                validateTokenOptionsWithAccessToken
            );
            expect(validateTokenSpy).toReturnWith(
                Promise.resolve(validateTokenResponse)
            );
        ...
        it("throws error if tokenType in response is not bearer", async () => {
            ...
            await validator
                .validateTokenFromResponse(msalResponse)
                .catch((e) => {
                    expect(e).toBeInstanceOf(ValidationConfigurationError);
                    expect(e.errorCode).toContain(
                        ValidationConfigurationErrorMessage
                            .invalidAuthenticationScheme.code
                    );
                    expect(e.errorMessage).toContain(
                        ValidationConfigurationErrorMessage
                            .invalidAuthenticationScheme.desc
                    );
                });
        });
    });
    ...
    describe("validateToken", () => {
        it("returns defaults", async () => {
            const getJWKSSpy = jest
                .spyOn(validator, "getJWKS")
                .mockReturnValue(
                    Promise.resolve(TEST_CONSTANTS.DEFAULT_JWKS_URI_OIDC)
                );
            ...
            expect(getJWKSSpy).toReturnWith(
                Promise.resolve(TEST_CONSTANTS.DEFAULT_JWKS_URI_OIDC)
            );

            expect(validateClaimsSpy).toHaveBeenCalledTimes(1);
            expect(validateClaimsSpy).toHaveBeenCalledWith(
                joseMockResult.payload,
        ...
        it("throws missing token error when token is empty string", async () => {
            ...
            await validator.validateToken("", testOptions).catch((e) => {
                expect(e).toBeInstanceOf(ValidationConfigurationError);
                expect(e.errorCode).toContain(
                    ValidationConfigurationErrorMessage.missingToken.code
                );
    ...
    describe("getJWKS", () => {
        ...
        it("calls createRemoteJWKSet with uri retrieved from metadata endpoint if no issuerSigningKeys or issuerSigningJwksUri provided in params", async () => {
            jest.spyOn(
                OpenIdConfigProvider.prototype,
                "fetchJwksUriFromEndpoint"
            ).mockReturnValue(
                Promise.resolve(TEST_CONSTANTS.DEFAULT_JWKS_URI_AAD)
            );
            const mock = mocked(createRemoteJWKSet);
            ...
            expect(mock).toHaveBeenCalledWith(
                new URL(TEST_CONSTANTS.DEFAULT_JWKS_URI_AAD)
            );
        });
    });
    ...
    describe("validateClaims", () => {
        it("throws error if nonce present in token but not passed in params", async () => {
            ...
            await validator
                .validateClaims(testPayload, defaultValidationParams)
                .catch((e) => {
                    expect(e).toBeInstanceOf(ValidationConfigurationError);
                    expect(e.errorCode).toContain(
        ...
        it("throws error if nonce in token and nonce in params do not match", async () => {
            ...
            await validator
                .validateClaims(testPayload, testOptions)
                .catch((e) => {
        ...
        it("does not check hash value if c_hash is present in token but code not provided in params", async () => {
            ...
            await validator.validateClaims(
                testPayload,
                defaultValidationParams
            );
        ...
        it("checks hash value if c_hash in token and code in params", async () => {
            ...
            expect(spy).toHaveBeenCalledWith(
                TEST_HASH_CONSTANTS.CODE,
                TEST_HASH_CONSTANTS.C_HASH
            );
        });
        ...
        it("throws error if c_hash in payload and code in params, but does not pass hash check", async () => {
            ...
            await validator
                ...
                .catch((e) => {
                    ...
                    expect(spy).toHaveBeenCalledTimes(1);
                    expect(spy).toHaveBeenCalledWith(
                        TEST_HASH_CONSTANTS.INVALID_CODE,
                        TEST_HASH_CONSTANTS.C_HASH
                    );
                });
        });
        ...
        it("does not check hash value if at_hash is not in token", async () => {
            const testOptions: BaseValidationParameters = {
                ...
                accessTokenForAtHash:
                    TEST_HASH_CONSTANTS.ACCESS_TOKEN_FOR_AT_HASH,
            };
            const spy = jest.spyOn(validator, "checkHashValue");
        ...
        it("throws error if at_hash in payload and access token in params, but does not pass hash check", async () => {
            ...
            const testOptions: BaseValidationParameters = {
                ...defaultValidationParams,
                accessTokenForAtHash:
                    TEST_HASH_CONSTANTS.INVALID_ACCESS_TOKEN_FOR_AT_HASH,
            };
            const spy = jest.spyOn(validator, "checkHashValue");
