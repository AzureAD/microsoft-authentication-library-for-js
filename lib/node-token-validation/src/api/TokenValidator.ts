...
} from "../config/TokenValidationParameters";
...
export class TokenValidator {
    ...
    constructor(configuration?: Configuration) {
        ...
        // Initialize storage
        this.storage = new NodeCacheManager(
            this.logger,
        ...
        (this.networkInterface = this.config.system.networkClient),
            ...
            (this.openIdConfigProvider = new OpenIdConfigProvider(
                this.config,
    ...
    validateTokenMiddleware(
        ...
        resource?: string
    ): TokenValidationMiddlewareResponse {
        return (
            req: ExpressRequest,
            ...
            if (resource && req.session && req.session.protectedResources) {
                if (!req.session.protectedResources[resource]) {
                    const error =
                        ValidationConfigurationError.createMissingTokenError(
                            `Resource ${resource} is not a protectedResource in req session. No access token to add to headers.`
                        );
                    next(error);
                } else {
                    const token =
                        req.session.protectedResources[resource].accessToken;
                    if (!token) {
                        const error =
                            ValidationConfigurationError.createMissingTokenError(
                                `No access token for resource ${resource} in session. Unable to add to request headers.`
                            );
                        next(error);
                    }
     ...
     */
    async validateTokenFromRequest(
        ...
        if (request.headers && request.headers.authorization) {
            ...
            if (
                ...
                this.logger.verbose(
                    "Bearer token extracted from request authorization headers"
                );
                return this.validateToken(token, options);
            } else {
                this.logger.verbose(
                    "Request authorization headers does not include bearer token"
                );
        ...
        throw ValidationConfigurationError.createMissingTokenError(
            "No tokens found in authorization header or body."
        );
    }
     ...
     */
    async validateTokenFromResponse(
        response: AuthenticationResult,
        idTokenOptions?: TokenValidationParameters,
        accessTokenOptions?: TokenValidationParameters
    ): Promise<TokenValidationResponse[]> {
        ...
        if (
            response.tokenType.toLowerCase() ===
            AuthenticationScheme.BEARER.toLowerCase()
        ) {
            this.logger.verbose(
                "TokenValidator - Bearer authentication scheme confirmed"
            );
            const validateResponse: TokenValidationResponse[] = [];

            if (!response.idToken && !response.accessToken) {
                this.logger.verbose(
                    "TokenValidator.validateTokenFromResponse - No tokens on response object to validate"
                );
            }
            ...
            if (response.idToken) {
                if (!idTokenOptions) {
                    this.logger.verbose(
                        "TokenValidator.validateTokenFromResponse - id token present on response but idTokenOptions not passed. Id token is not validated"
                    );
                } else {
                    this.logger.verbose(
                        "TokenValidator.validateTokenFromResponse - id token present on response, validating"
                    );
                    ...
                    if (response.accessToken) {
                        idTokenOptions.accessTokenForAtHash =
                            response.accessToken;
                    }

                    const validateIdTokenResponse: TokenValidationResponse =
            ...
            if (response.accessToken) {
                if (!accessTokenOptions) {
                    this.logger.verbose(
                        "TokenValidator.validateTokenFromResponse - access token present on response but idTokenOptions not passed. Access token is not validated"
                    );
                } else {
                    this.logger.verbose(
                        "TokenValidator.validateTokenFromResponse - access token present on response, validating"
                    );
                    const validateAccessTokenResponse: TokenValidationResponse =
                        await this.validateToken(
                            response.accessToken,
                            accessTokenOptions
                        );
            ...
            throw ValidationConfigurationError.createInvalidAuthenticationScheme(
                "TokenType in response is not bearer."
            );
        }
    }
    ...
    async validateToken(
        ...
        const validationParams: BaseValidationParameters =
            await buildTokenValidationParameters(options);
    ...
    async getJWKS(validationParams: BaseValidationParameters): Promise<any> {
        ...
        if (validationParams.issuerSigningJwksUri) {
            ...
            return createRemoteJWKSet(
                new URL(validationParams.issuerSigningJwksUri)
            );
        }

        // Do resiliency well-known endpoint thing here
        const retrievedJwksUri: string =
            await this.openIdConfigProvider.fetchJwksUriFromEndpoint();
    ...
    setIssuerParams(options: BaseValidationParameters): string[] {
        ...
        if (
            options.validIssuers.length < 1 ||
            options.validIssuers[0].length < 1
        ) {
    ...
    setAudienceParams(options: BaseValidationParameters): string[] {
        ...
        if (
            options.validAudiences.length < 1 ||
            options.validAudiences[0].length < 1
        ) {
    ...
    async validateClaims(
        payload: JWTPayload,
        validationParams: BaseValidationParameters
    ): Promise<void> {
        ...
        if (payload.c_hash && typeof payload.c_hash === "string") {
            ...
            if (!validationParams.code) {
                this.logger.verbose(
                    "C_hash present on token but code not set in validationParams. Unable to validate c_hash"
                );
            } else {
                const hashResult = await this.checkHashValue(
                    validationParams.code,
                    payload.c_hash
                );
                if (!hashResult) {
        ...
        if (payload.at_hash && typeof payload.at_hash === "string") {
            ...
            if (!validationParams.accessTokenForAtHash) {
                this.logger.verbose(
                    "At_hash present on token but access token not set in validationParams. Unable to validate at_hash"
                );
            } else {
                const hashResult = await this.checkHashValue(
                    validationParams.accessTokenForAtHash,
                    payload.at_hash
                );
                if (!hashResult) {
