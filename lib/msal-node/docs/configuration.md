...
| `customAgent`        | A http(s) proxy agent                                 | http.Agent or https.Agent - [NodeJS documentation on proxy agents](https://nodejs.org/docs/latest-v16.x/api/http.html#class-httpagent) | Proxy Agent `new http.Agent()`, `new https.Agent()` or a proxy agent created by a third-party proxy agent library                    |
