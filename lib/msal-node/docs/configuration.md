...
| `disableInternalRetries` | A boolean that, if enabled, disables MSALJS's built-in retry policies, allowing the app developer to specify their own retry policy. Currently, only Managed Identity flows have a simple retry policy of retrying once on certain HTTP error codes. | boolean                                                                                                                    | boolean `false`                                                                                                                      |
