...
// Store the nonce and state with the authentication transaction so the same
// values can be provided to acquireTokenByCode after the redirect.
...
cca.acquireTokenByCode(tokenRequest, authCodeResponse)
    .then((response) => {
