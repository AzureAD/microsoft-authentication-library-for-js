...
-   **`mtls_binding_certificate_missing` (`NodeAuthError`).** `mtlsProofOfPossession: true` was requested but no usable binding certificate is resolvable. Configure `auth.clientCertificate` — or pass a request-level `tokenBindingCertificate` (FIC Leg 2) — with **both** an `x5c` (certificate or chain) and a `privateKey`; a thumbprint-only certificate is not sufficient for mTLS PoP.
