...
describe("Serializer test cases", () => {
    ...
    test("serializeAccountCacheEntity", () => {
        ...
        expect(serializedAcc[MockCache.accKey]).toMatchObject(
            jsonCache.Account[MockCache.accKey]
        );
    });
    ...
    test("serializeIdTokenCacheEntity", () => {
        ...
        expect(serializedIdT[MockCache.idTKey]).toMatchObject(
            jsonCache.IdToken[MockCache.idTKey]
        );
    });
    ...
    test("serializeAccessTokenEntity", () => {
        ...
        expect(serializedAt[MockCache.atOneKey]).toMatchObject(
            jsonCache.AccessToken[MockCache.atOneKey]
        );
    });
    ...
    test("serializeRefreshTokenCacheEntity", () => {
        ...
        expect(serializedRT[MockCache.rtKey]).toMatchObject(
            jsonCache.RefreshToken[MockCache.rtKey]
        );
    });
    ...
    test("serializeAppMetadataCacheEntity", () => {
        ...
        expect(serializedAmdt[MockCache.amdtKey]).toMatchObject(
            jsonCache.AppMetadata[MockCache.amdtKey]
        );
    });
    ...
    test("serializeAll", () => {
        ...
        expect(jCache.Account[MockCache.accKey]).toMatchObject(
            jsonCache.Account[MockCache.accKey]
        );
        expect(jCache.IdToken[MockCache.idTKey]).toMatchObject(
            jsonCache.IdToken[MockCache.idTKey]
        );
        expect(jCache.AccessToken[MockCache.atOneKey]).toMatchObject(
            jsonCache.AccessToken[MockCache.atOneKey]
        );
        expect(jCache.AccessToken[MockCache.atTwoKey]).toMatchObject(
            jsonCache.AccessToken[MockCache.atTwoKey]
        );
        expect(jCache.RefreshToken[MockCache.rtKey]).toMatchObject(
            jsonCache.RefreshToken[MockCache.rtKey]
        );
        expect(jCache.RefreshToken[MockCache.rtFKey]).toMatchObject(
            jsonCache.RefreshToken[MockCache.rtFKey]
        );
        expect(jCache.AppMetadata[MockCache.amdtKey]).toMatchObject(
            jsonCache.AppMetadata[MockCache.amdtKey]
        );
    });
});
