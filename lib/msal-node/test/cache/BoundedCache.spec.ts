...
describe("bounded Node token cache", () => {
    ...
    it("evicts credentials by count without evicting accounts or unknown records", async () => {
        ...
        expect(storage.getItem(generateCredentialKey(second))).toEqual(second);
        expect(storage.getItem(generateCredentialKey(third))).toEqual(third);
        expect(storage.getItem("account-key")).toEqual(account);
    ...
    it("promotes a selected structural cache hit even when the token is expired", async () => {
        ...
        await storage.setAccessTokenCredential(third, "", false);
        expect(storage.getItem(generateCredentialKey(first))).toEqual(first);
    ...
    it("promotes the exact access-token key with precomputed additional components", async () => {
        ...
        expect(storage.getItem(selectedKey)).toEqual(selected);
        expect(
            storage.getAccessToken(
                {
    ...
    it("applies one shared entry limit to access, refresh, and ID tokens", async () => {
        ...
        expect(storage.getItem(generateCredentialKey(refreshToken))).toEqual(
            refreshToken
        );
        expect(storage.getItem(generateCredentialKey(accessToken))).toEqual(
            accessToken
        );
    });
    ...
    it("retains credential-shaped records with an unknown credential type", async () => {
        ...
        expect(storage.getItem("future-credential")).toEqual(unknown);
    });
    ...
    it("rejects an oversized replacement without evicting unrelated credentials", async () => {
        ...
        expect(storage.getItem(secondKey)).toEqual(second);
    });
    ...
    it("retains a shared token-binding key until its final credential is evicted", async () => {
        const removeTokenBindingKey = jest.fn(async () => undefined);
        const storage = createStorage(2, 1024 * 1024, false, {
            ...DEFAULT_TOKEN_BINDING_KEY_MANAGER,
            removeTokenBindingKey,
        });
        const firstBoundToken = {
            ...createAccessToken("scope.bound.first"),
            credentialType:
                Constants.CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME,
            tokenType: Constants.AuthenticationScheme.POP,
            keyId: "shared-binding-key",
        };
        const secondBoundToken = {
            ...createAccessToken("scope.bound.second"),
            credentialType:
                Constants.CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME,
            tokenType: Constants.AuthenticationScheme.DPOP,
            keyId: "shared-binding-key",
        };
        await storage.setAccessTokenCredential(firstBoundToken, "", false);
        await storage.setAccessTokenCredential(secondBoundToken, "", false);
        ...
        expect(
            storage.getItem(generateCredentialKey(secondBoundToken))
        ).toEqual(secondBoundToken);

        await storage.setAccessTokenCredential(
            createAccessToken("scope.final"),
            "",
            false
        );
        await Promise.resolve();
    ...
    it("serializes overlapping plugin loads and persists the owning snapshot", async () => {
        const first = createAccessToken("scope.first");
        const second = createAccessToken("scope.second");
        const third = createAccessToken("scope.third");
        ...
        const continueFirstLoad = new Promise<void>((resolve) => {
        ...
        expect(Object.keys(JSON.parse(persisted).AccessToken)).toEqual([
            generateCredentialKey(second),
            generateCredentialKey(third),
        ]);
    });
});
