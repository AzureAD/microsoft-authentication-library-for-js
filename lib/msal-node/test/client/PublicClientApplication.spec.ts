...
describe("PublicClientApplication", () => {
    ...
    describe("acquireTokenSilent tests", () => {
        ...
        test("acquireTokenSilent finds sovereign cloud token when PCA authority is public cloud", async () => {
            const sovereignHost = "login.microsoftonline.us";
            const sovereignAuthority = `https://${sovereignHost}/common/`;

            // OpenID configs for each cloud
            const publicOpenIdConfig = DEFAULT_OPENID_CONFIG_RESPONSE.body;
            const sovereignOpenIdConfig = {
                ...publicOpenIdConfig,
                token_endpoint: `https://${sovereignHost}/{tenant}/oauth2/v2.0/token`,
                issuer: `https://${sovereignHost}/{tenant}/v2.0`,
                authorization_endpoint: `https://${sovereignHost}/{tenant}/oauth2/v2.0/authorize`,
                end_session_endpoint: `https://${sovereignHost}/{tenant}/oauth2/v2.0/logout`,
            };

            // Token responses for each call
            ...
            const sovereignTokenResponse = {
                status: 200,
                body: {
                    ...AUTHENTICATION_RESULT.body,
                    access_token: "sovereign-access-token",
                    refresh_token: "sovereign-refresh-token",
                    client_info:
                        TEST_DATA_CLIENT_INFO.TEST_RAW_CLIENT_INFO,
                },
                headers: {},
            ...
            // Mock authority resolution — return correct config based on the authority host
            const endpointSpy = jest.spyOn(
                Authority.prototype,
                <any>"getEndpointMetadataFromNetwork"
            );

            // Mock token endpoint POST — return correct token based on call order
            const postSpy = jest.spyOn(
                HttpClient.prototype,
                "sendPostRequestAsync"
            );

            // Simulate a persistent cache (like msal-node-extensions)
            // Stores serialized cache on afterCacheAccess, restores on beforeCacheAccess
            let persistedCacheData: string = "";
            const cachePlugin = {
                beforeCacheAccess: jest
                    .fn()
                    .mockImplementation(
            ...
            // Create PCA with public cloud authority and persistent cache
            const authApp = new PublicClientApplication({
                ...appConfig,
                cache: { cachePlugin },
            });

            // Step 1: acquireTokenByCode with public cloud authority
            endpointSpy.mockResolvedValueOnce(publicOpenIdConfig);
            postSpy.mockResolvedValueOnce(publicTokenResponse);

            const publicAuthCodeRequest: AuthorizationCodeRequest = {
                scopes: TEST_CONSTANTS.DEFAULT_GRAPH_SCOPE,
                redirectUri: TEST_CONSTANTS.REDIRECT_URI,
                code: "fake-public-auth-code",
            };
            const publicResult = await authApp.acquireTokenByCode(
                publicAuthCodeRequest
            );
            expect(publicResult.accessToken).toEqual(
                "public-access-token"
            );
            expect(publicResult.fromCache).toBe(false);

            // Step 2: acquireTokenByCode with sovereign authority
            endpointSpy.mockResolvedValueOnce(sovereignOpenIdConfig);
            postSpy.mockResolvedValueOnce(sovereignTokenResponse);

            const sovereignAuthCodeRequest: AuthorizationCodeRequest =
                {
                    scopes: TEST_CONSTANTS.DEFAULT_GRAPH_SCOPE,
                    redirectUri: TEST_CONSTANTS.REDIRECT_URI,
                    code: "fake-sovereign-auth-code",
                    authority: sovereignAuthority,
                };
            const sovereignResult = await authApp.acquireTokenByCode(
                sovereignAuthCodeRequest
            );
            expect(sovereignResult.accessToken).toEqual(
                "sovereign-access-token"
            );
            expect(sovereignResult.fromCache).toBe(false);

            // Step 3: acquireTokenSilent with sovereign authority
            // overwriteCache() will clear in-memory storage and restore from
            // the persistent cache, which doesn't include authority metadata
            endpointSpy.mockResolvedValueOnce(sovereignOpenIdConfig);

            const accounts = await authApp.getAllAccounts();
            const sovereignAccount = accounts.find(
                (a) => a.environment === sovereignHost
            );
            expect(sovereignAccount).toBeDefined();
            ...
            const silentResult =
                await authApp.acquireTokenSilent(silentRequest);
            expect(silentResult.accessToken).toEqual(
                "sovereign-access-token"
            );
            expect(silentResult.fromCache).toBe(true);
        });
    });
