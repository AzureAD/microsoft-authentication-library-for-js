...
export class NodeStorage extends CacheManager {
    ...
    private tokenBindingKeyReferences = new Map<string, number>();
    ...
    getCacheSnapshot(): CacheKVStore {
        return this.cloneCacheValue(this.cache);
    }

    private cloneCacheValue<T>(value: T): T {
        if (Array.isArray(value)) {
            return value.map((item) => this.cloneCacheValue(item)) as T;
        }
        if (value && typeof value === "object") {
            const clone: Record<string, unknown> = {};
            Object.keys(value).forEach((key) => {
                clone[key] = this.cloneCacheValue(
    ...
    private rebuildIndexes(
        ...
        this.tokenBindingKeyReferences.clear();
    ...
    private addToIndexes(key: string, value: ValidCacheType): void {
        ...
        if (AccountEntityUtils.isAccountEntity(value)) {
            ...
            this.addTokenBindingKeyReference(value);
    ...
    private removeFromIndexes(key: string, value: ValidCacheType): void {
        ...
        if (AccountEntityUtils.isAccountEntity(value)) {
            ...
            this.removeTokenBindingKeyReference(value);
        } else if (CacheHelpers.isRefreshTokenEntity(value)) {
    ...
    private getTokenBindingKeyId(
        ...
        if (
            tokenType !== Constants.AuthenticationScheme.POP.toLowerCase() &&
            tokenType !== Constants.AuthenticationScheme.DPOP.toLowerCase()
        ) {
            return undefined;
        }
    ...
    private addTokenBindingKeyReference(credential: AccessTokenEntity): void {
        const keyId = this.getTokenBindingKeyId(credential);
        if (keyId) {
            this.tokenBindingKeyReferences.set(
                keyId,
                (this.tokenBindingKeyReferences.get(keyId) || 0) + 1
            );
        }
    }

    private removeTokenBindingKeyReference(
        ...
        const referenceCount =
            (this.tokenBindingKeyReferences.get(keyId) || 0) - 1;
        if (referenceCount > 0) {
            this.tokenBindingKeyReferences.set(keyId, referenceCount);
        } else {
            this.tokenBindingKeyReferences.delete(keyId);
        }
    }
    ...
    protected removeTokenBindingKeyForCredential(
        ...
        const keyId = this.getTokenBindingKeyId(credential);
        if (keyId && (this.tokenBindingKeyReferences.get(keyId) || 0) > 0) {
            return;
        }

        super.removeTokenBindingKeyForCredential(credential, correlationId);
    }
    ...
    private getEnvironmentAliases(
        ...
        const metadata = this.getAuthorityMetadataByAliasFromIndex(environment);
        if (metadata?.aliases.includes(environment)) {
    ...
    private getAccessTokenCandidates(
        ...
        if (
            !filter.tokenType &&
            this.normalizeString(filter.credentialType) ===
                Constants.CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME.toLowerCase()
        ) {
            return null;
        }
    ...
    getInMemoryCache(): InMemoryCache {
        ...
        const inMemoryCache = this.cacheToInMemoryCache(
    ...
    setCache(cache: CacheKVStore): void {
        ...
        this.cache = this.cloneCacheValue(cache);
        try {
    ...
    setItem(key: string, value: ValidCacheType): void {
        ...
        const ownedValue = this.cloneCacheValue(value);
        ...
        try {
            if (
                ownedValue &&
                typeof ownedValue === "object" &&
                this.isBoundedCredential(ownedValue)
            ) {
                this.getCredentialLogicalWeight(key, ownedValue);
            }
            if (hadItem) {
                if (
                    previousValue &&
            ...
            cache[key] = ownedValue;
            this.addToIndexes(key, ownedValue);
            if (
                ownedValue &&
                typeof ownedValue === "object" &&
                this.isBoundedCredential(ownedValue)
            ) {
                this.admitCredential(key, ownedValue);
            }
        } catch (error) {
            if (hadItem) {
    ...
    getAccountKeys(): string[] {
        return this.orderCandidateKeys(this.accountKeys);
    }

    getTokenKeys(): TokenKeys {
        return {
            idToken: this.orderCandidateKeys(this.idTokenKeys),
            accessToken: this.orderCandidateKeys(this.accessTokenKeys),
            refreshToken: this.orderCandidateKeys(this.refreshTokenKeys),
        };
    }
    ...
    getAuthorityMetadataKeys(): Array<string> {
        return this.orderCandidateKeys(this.authorityMetadataKeys);
    }
    ...
    setAuthorityMetadata(key: string, metadata: AuthorityMetadataEntity): void {
        const ownedMetadata = this.cloneCacheValue(metadata);
        const snapshot = this.stableStringify(ownedMetadata);
        ...
        try {
            this.removeAuthorityMetadataFromIndexes(key);
            this.cache[key] = ownedMetadata;
            if (previousOrder !== undefined) {
                this.cacheKeyOrder.set(key, previousOrder);
            } else {
                this.cacheKeyOrder.set(key, this.nextCacheKeyOrder++);
            }
            this.addAuthorityMetadataToIndexes(key, ownedMetadata, snapshot);
        } catch (error) {
            if (hadItem) {
