 ...
 * - clientCertificate      - Certificate that the application uses when requesting a token. Only used in confidential client applications. Requires hex encoded X.509 SHA-1 or SHA-256 thumbprint of the certificate, and the PEM encoded private key (string should contain -----BEGIN PRIVATE KEY----- ... -----END PRIVATE KEY----- ). A Subject Name + Issuer (SN/I) certificate may be used either to sign a client assertion (yielding a Bearer token) or, when a request sets mtlsProofOfPossession, as the client TLS certificate in the mutual-TLS handshake to obtain an mTLS-bound Proof-of-Possession (mtls_pop) token. See the mtls-proof-of-possession.md doc.
 * @public
 */
