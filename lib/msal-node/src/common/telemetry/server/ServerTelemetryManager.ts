...
export class ServerTelemetryManager {
    ...
    cacheFailedRequest(error: unknown): void {
        try {
            ...
            if (error instanceof Error && error.toString()) {
                const authError = error as Error &
                    Partial<Pick<AuthError, "errorCode" | "subError">>;
                if (
                    typeof authError.subError === "string" &&
                    authError.subError
                ) {
                    lastRequests.errors.push(authError.subError);
                } else if (
                    typeof authError.errorCode === "string" &&
                    authError.errorCode
                ) {
                    lastRequests.errors.push(authError.errorCode);
                } else {
