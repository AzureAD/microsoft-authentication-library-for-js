...
Note: Passing the "Silent" interaction type will call `ssoSilent` which attempts to open a hidden iframe and reuse an existing session with AAD. This will not work in browsers that block 3rd party cookies such as Safari. The request object is optional when using the "Silent" type — if no request is provided, an empty request object will be used. However, for best reliability it is strongly recommended to provide the user's sign-in information via `loginHint`, `sid`, or `account` so that the correct session can be located. Note: there are [additional considerations](https://github.com/AzureAD/microsoft-authentication-library-for-js/blob/dev/lib/msal-browser/docs/login-user.md#silent-login-with-ssosilent) when using `ssoSilent` without providing any information about the user's session.
