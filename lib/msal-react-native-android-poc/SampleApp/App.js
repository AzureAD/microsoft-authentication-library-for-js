...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      aId: null,
      aAccessToken: null,
      aAuthenticationScheme: null,
      aAuthorizationHeader: null,
      aExpiresOn: null,
      aScope: null,
      graphResult: null
    };
      ...
      <>
        ...
        <SafeAreaView style={styles.SafeAreaView}>
          ...
          contentContainerStyle={{justifyContent: "space-around"}} >
                  ...
                  onPress = {async () => {
                    try {
                      var {
                        ...
                        expiresOn,
                        scope
                      } = await MSAL.acquireTokenSilent(this.state.scopesValue);
                      this.setState({
                        aAccessToken: accessToken,
                        aAuthenticationScheme: authenticationScheme,
                        aAuthorizationHeader: authorizationHeader,
                        aExpiresOn: expiresOn,
                        aScope: scope
                      });
                      //make api call
                      var response = await fetch (
                        'https://graph.microsoft.com/v1.0/me', {
                          method: 'GET',
                          headers: {
                            'Authorization': this.state.aAccessToken
                          }
                        }
                      );
           ...
           <View
           style={styles.viewResult}>
             <Text>{this.state.graphResult != null ? JSON.stringify(this.state.graphResult, null, 4) : ""}</Text>
           </View>
