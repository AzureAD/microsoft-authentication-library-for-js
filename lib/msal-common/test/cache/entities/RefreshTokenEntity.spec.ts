...
describe("RefreshTokenEntity.ts Unit Tests", () => {
    ...
    describe("RefreshToken_With_AuthScheme credential entity", () => {
        it("Verify an RefreshToken_With_AuthScheme entity", () => {
            const popRT = mockCache.createMockPopRT();
            expect(popRT instanceof RefreshTokenEntity);
        });

        it("Generate RefreshTokenEntity key", () => {
            const popRT = mockCache.createMockPopRT();
            expect(popRT.generateCredentialKey()).toEqual(
                "uid.utid-login.microsoftonline.com-refreshtoken_with_authscheme-mock_client_id---pop"
            );
        });

        it("Generate RefreshTokenEntity type", () => {
            const popRT = mockCache.createMockPopRT();
            expect(popRT.generateType()).toEqual(CacheType.REFRESH_TOKEN);
        });

        it("verify if an object is an access token entity", () => {
            expect(RefreshTokenEntity.isRefreshTokenEntity(mockRefreshTokenWithAuthSchemeEntity)).toEqual(true);
        });
    });
});
