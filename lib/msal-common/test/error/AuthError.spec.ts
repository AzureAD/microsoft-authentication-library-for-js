...
describe("AuthError.ts Class", () => {
    ...
    it("AuthError object can be created", () => {
        ...
        expect(err.stack).toEqual(expect.arrayContaining(["AuthError.spec.ts"]));
    });
    ...
    it("createUnexpectedError creates a AuthError object", () => {
        ...
        expect(err.errorMessage).toEqual(expect.arrayContaining([AuthErrorMessage.unexpectedError.desc]));
        expect(err.errorMessage).toEqual(expect.arrayContaining([ERROR_DESC]));
        expect(err.message).toEqual(expect.arrayContaining([AuthErrorMessage.unexpectedError.desc]));
        expect(err.name).toBe("AuthError");
        expect(err.stack).toEqual(expect.arrayContaining(["AuthError.spec.ts"]));
    });
});
