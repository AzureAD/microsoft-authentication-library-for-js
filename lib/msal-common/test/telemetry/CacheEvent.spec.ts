...
describe("CacheEvent", () => {
    it("constructs and carries exepcted values", () => {
        ...
        const event = new CacheEvent(CryptoUtils.createNewGuid(), CACHE_EVENT_TYPES.TokenCacheLookup, correlationId).get();
    ...
    it("sets values", () =>{
        const correlationId = CryptoUtils.createNewGuid();
        const cacheEvent = new CacheEvent(CryptoUtils.createNewGuid(), CACHE_EVENT_TYPES.TokenCacheBeforeAccess, correlationId);
