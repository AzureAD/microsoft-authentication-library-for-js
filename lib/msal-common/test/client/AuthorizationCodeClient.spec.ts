...
describe("AuthorizationCodeClient unit tests", () => {
    ...
    describe("Acquire a token", () => {
        ...
        it("preventCorsPreflight=true does not add headers other than simpleRequest headers", async () => {
            ...
            const headers = executePostToTokenEndpointSpy.mock.lastCall[2];
            const headerNames = Object.keys(headers);
            expect(headerNames.length).toBeGreaterThanOrEqual(1);
        ...
        it("preventCorsPreflight=false adds headers to request", async () => {
            ...
            const headers = executePostToTokenEndpointSpy.mock.lastCall[2];
            const headerNames = Object.keys(headers);
            expect(headerNames.length).toBeGreaterThanOrEqual(2);
            headerNames.forEach((name) => {
        ...
        it("Acquires a token successfully", async () => {
            ...
            const createTokenRequestBodySpy: jest.SpyInstance = jest.spyOn(
                AuthorizationCodeClient.prototype,
        ...
        it("Adds tokenQueryParameters to the /token request", async () => {
            ...
            const executePostToTokenEndpointSpy: jest.SpyInstance = jest.spyOn(
                AuthorizationCodeClient.prototype,
                <any>"executePostToTokenEndpoint"
            );
            ...
            await expect(
                client.acquireToken(authorizationCodeRequest)
            ).rejects.toThrow();
        ...
        it("Adds tokenBodyParameters to the /token request", async () => {
            jest.spyOn(
                Authority.prototype,
                <any>"getEndpointMetadataFromNetwork"
            ).mockResolvedValue(DEFAULT_OPENID_CONFIG_RESPONSE.body);
            ...
            const body: string = executePostToTokenEndpointSpy.mock
                .lastCall[1] as string;
            expect(body.includes("extra_body_parameter=true")).toBeTruthy();
        });

        it("Adds return_spa_code=1 to body when enableSpaAuthCode is set", async () => {
            jest.spyOn(
                Authority.prototype,
                <any>"getEndpointMetadataFromNetwork"
            ).mockResolvedValue(DEFAULT_OPENID_CONFIG_RESPONSE.body);
            ...
            const body: string = executePostToTokenEndpointSpy.mock
                .lastCall[1] as string;
            expect(body.includes("return_spa_code=1")).toBeTruthy();
        });

        it("Doesnt add redirect_uri when hybridSpa flag is set", async () => {
            ...
            const body: string = executePostToTokenEndpointSpy.mock
                .lastCall[1] as string;
            expect(body.includes("redirect_uri=")).toBeFalsy();
        });
