...
Because of this, any request parameter that influences the **identity or audience of the returned token** (for example `scopes`, `authority`, `claims`, `authenticationScheme`, `resource`) **must** be represented in the `RequestThumbprint` type and assigned to the object returned by `getRequestThumbprint`. Updating the type alone does not change the serialized deduplication key. Omitting such a field from the returned object lets two genuinely distinct concurrent requests collide on one key, so one caller receives a token minted for the other request (cross-resource / cross-audience token substitution and bearer-token disclosure).
