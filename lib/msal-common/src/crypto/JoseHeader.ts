...
export type SessionKeyJweHeader = JweHeader;

export interface ResponseJweHeader extends JweHeader {
    ctx: string;
    label: string;
}
...
export class JoseHeader {
    ...
    /**
     * Builds session_key_jwe formatted JOSE Header from the
     * JOSE Header options provided and returns the header object.
     * Throws if alg or enc claims aren't provided since they are required for session key unwrapping
     * @param jweHeaderOptions
     * @returns 
     */
    static getSessionKeyJweHeader(jweHeaderOptions: JoseHeaderOptions): SessionKeyJweHeader {
        // KeyID is required on the SHR header
    ...
    /**
     * Builds response_jwe formatted JOSE Header from the
     * JOSE Header options provided and returns the header object.
     * Throws if alg or enc claims aren't provided since they are required for Refresh Token Binding.
     * @param jweHeaderOptions
     * @returns 
     */
    static getResponseJweHeader(jweHeaderOptions: JoseHeaderOptions): ResponseJweHeader {
        // KeyID is required on the SHR header
        if (!jweHeaderOptions.alg) {
            throw JoseHeaderError.createMissingClaimError(JoseClaims.ALG);
        }
        ...
        // Ctx is required on the SHR header
        if (!jweHeaderOptions.ctx) {
            throw JoseHeaderError.createMissingClaimError(JoseClaims.CTX);
        }

        return new JoseHeader(jweHeaderOptions) as ResponseJweHeader;
    }
}
