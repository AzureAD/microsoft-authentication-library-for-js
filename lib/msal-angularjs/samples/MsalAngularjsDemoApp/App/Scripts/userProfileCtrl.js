...
angular.module('todoApp')
    .controller('userProfileCtrl', ['$scope', 'msalAuthenticationService', '$http', function ($scope, msalService, $http) {
        ...
        $scope.$on("msal:acquireTokenFailure", function (event, errorDesc, error) {
            if (error.indexOf("consent_required") !== -1 || eerror.indexOf("interaction_required") != -1 ) {
                msalService.acquireTokenPopup(applicationConfig.userProfileEndpoint).then(function (token) {
                    getUserProfileData();
                }, function (error) {
                });
            }
