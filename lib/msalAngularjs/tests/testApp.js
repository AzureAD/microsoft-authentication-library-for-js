...
app.config(['$httpProvider', '$routeProvider', 'msalAuthenticationServiceProvider', function ($httpProvider, $routeProvider, msalAuthenticationServiceProvider) {
...
app.controller('TaskCtl', ['$scope', '$location', 'msalAuthenticationService', 'TaskFactory', 'ItemFactory', function ($scope, $location, msalAuthenticationService, TaskFactory, ItemFactory) {
    ...
    $scope.taskCall5 = function () {
        TaskFactory.getItem2('https://localhost:8080/someapi/item').then(function (response) {
            $scope.task = response.data;
        }, function (err) {
            $scope.error = err;
            $scope.loadingMsg = "";
