...
export class MsalService extends UserAgentApplication {
    ...
    login_redirect(consentScopes, extraQueryParameters) {
        this._logger.verbose("login redirect flow");
        this.loginRedirect(consentScopes, extraQueryParameters);
    }
    login_popup(consentScopes, extraQueryParameters) {
    ...
    getCached_Token_Internal(scopes) {
        return this.getCachedTokenInternal(scopes, this.getUser());
    }
    acquire_token_silent(scopes, authority, user, extraQueryParameters) {
        return new Promise((resolve, reject) => {
            this.acquireTokenSilent(scopes, authority, user, extraQueryParameters).then((token) => {
                this._renewActive = false;
                this.broadcastService.broadcast('msal:acquireTokenSuccess', token);
                resolve(token);
            }, (error) => {
    ...
    acquire_token_popup(scopes, authority, user, extraQueryParameters) {
        return new Promise((resolve, reject) => {
            this.acquireTokenPopup(scopes, authority, user, extraQueryParameters).then((token) => {
                this._renewActive = false;
                this.broadcastService.broadcast('msal:acquireTokenSuccess', token);
                resolve(token);
            }, (error) => {
    ...
    acquire_token_redirect(scopes, authority, user, extraQueryParameters) {
