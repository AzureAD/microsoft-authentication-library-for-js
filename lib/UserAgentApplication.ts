    ...
    export class UserAgentApplication {
        ...
        login(extraQueryParameters?: Array<string>): void {
            ...
            */
            ...
            let authenticationRequest: AuthenticationRequestParameters = new AuthenticationRequestParameters(this.authority, this.clientId, null, ResponseTypes.id_token, this.redirectUri);
        ...
        getAllUsers(): Array<User> {
            ...
            for (let i = 0; i < accessTokenCacheItems.length; i++) {
                ...
                let user: User = User.createUser(idToken, clientInfo, this.authority);
                users.push(user);
            }
            return this.getUniqueUsers(users);
        }

        getUniqueUsers(users: Array<User>): Array<User> {
            if (users.length <= 1)
                return users;
        ...
        acquireToken(scopes: Array<string>, callback: (errorDesc: string, token: string, error: string) => void, user?: User, extraQueryParameters?: Array<string>): void {
            ...
            let scope: string = scopes.join(' ').toLowerCase();
            if (!userObject) {
            ...
            var authenticationRequest: AuthenticationRequestParameters;
            if (Utils.compareObjects(userObject, this.user)) {
                authenticationRequest = new AuthenticationRequestParameters(this.authority, this.clientId, scopes, ResponseTypes.token, this.redirectUri);
            }
            else {
                authenticationRequest = new AuthenticationRequestParameters(this.authority, this.clientId, scopes, ResponseTypes.id_token_token, this.redirectUri);
            }

            authenticationRequest.state = authenticationRequest.state + '|' + scope;
            this._cacheStorage.saveItem(Constants.nonceIdToken, authenticationRequest.nonce);
            let acquireTokenUserKey: string = Constants.acquireTokenUser + Constants.resourceDelimeter + userObject.userIdentifier + Constants.resourceDelimeter + authenticationRequest.state;
        ...
        acquireTokenSilent(scopes: Array<string>, callback: (errorDesc: string, token: string, error: string) => void, user?: User, extraQueryParameters?: Array<string>): void {
            var isValidScope = this.validateInputScope(scopes);
            if (isValidScope && !Utils.isEmpty(isValidScope)) {
                callback(isValidScope, null, null);
                return;
        ...
        private renewToken(scopes: Array<string>, callback: Function, user: User, extraQueryParameters?: Array<string>): void {
            ...
            var authenticationRequest: AuthenticationRequestParameters;
            if (Utils.compareObjects(user, this.user)) {
                authenticationRequest = new AuthenticationRequestParameters(this.authority, this.clientId, scopes, ResponseTypes.token, this.redirectUri);
            }
            else {
                authenticationRequest = new AuthenticationRequestParameters(this.authority, this.clientId, scopes, ResponseTypes.id_token_token, this.redirectUri);
            ...
            let acquireTokenUserKey: string = Constants.acquireTokenUser + Constants.resourceDelimeter + user.userIdentifier + Constants.resourceDelimeter + authenticationRequest.state;
            if (Utils.isEmpty(this._cacheStorage.getItem(acquireTokenUserKey))) {
        ...
        private renewIdToken(scopes: Array<string>, callback: Function, user: User, extraQueryParameters?: Array<string>): void {
        ...
        private saveAccessToken(requestInfo: RequestInfo, user: User, clientInfo: string, idToken: IdToken): void {
            let userObject = user ? user : this.user;
            let scope: string;
            if (requestInfo.parameters.hasOwnProperty('scope')) {
                ...
                let accessTokenKey = new AccessTokenKey(user.identityProvider, this.clientId, scope, user.uid, user.utid);
                let accessTokenValue = new AccessTokenValue(requestInfo.parameters[Constants.accessToken], idToken.getRawIdToken, Utils.expiresIn(requestInfo.parameters[Constants.expiresIn]).toString(), clientInfo);
                this._cacheStorage.saveItem(JSON.stringify(accessTokenKey), JSON.stringify(accessTokenValue));
            }
            else {
                scope = this.clientId;
                let accessTokenKey = new AccessTokenKey(user.identityProvider, this.clientId, scope, user.uid, user.utid);
                let accessTokenValue = new AccessTokenValue(requestInfo.parameters[Constants.idToken], requestInfo.parameters[Constants.idToken], idToken.expiration, clientInfo);
                this._cacheStorage.saveItem(JSON.stringify(accessTokenKey), JSON.stringify(accessTokenValue));
            }
                ...
                if (requestInfo.stateMatch) {
                    ...
                    if (requestInfo.parameters.hasOwnProperty(Constants.accessToken)) {
                        ...
                        if (requestInfo.parameters.hasOwnProperty(Constants.idToken)) {
                            idToken = new IdToken(requestInfo.parameters[Constants.idToken]);
                        }
                        else {
                        ...
                        if (requestInfo.parameters.hasOwnProperty(Constants.clientInfo)) {
                            ...
                            user = User.createUser(idToken, new ClientInfo(clientInfo),this.authority);
                        }
                        else {
                        ...
                        let acquireTokenUserKey: string = Constants.acquireTokenUser + Constants.resourceDelimeter + user.userIdentifier + Constants.resourceDelimeter + requestInfo.stateResponse;
                    ...
                    if (requestInfo.parameters.hasOwnProperty(Constants.idToken)) {
                        if (scope.indexOf(this.clientId) > -1) {
                            ...
                            this.user = User.createUser(idToken, new ClientInfo(clientInfo), this.authority);
