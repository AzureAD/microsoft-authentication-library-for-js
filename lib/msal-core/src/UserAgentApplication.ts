...
export class UserAgentApplication {
    ...
    private redirectResponse: AuthResponse|null;
     ...
     */
    loginRedirect(userRequest?: AuthenticationParameters|undefined): void {
        // validate request
    ...
    private acquireTokenInteractive(interactionType: InteractionType, isLoginCall: boolean, request: AuthenticationParameters, resolve?: any, reject?: any): void {
        ...
        const account: Account|null = (request && request.account && !isLoginCall) ? request.account : this.getAccount();
    ...
    private async acquireTokenHelper(account: Account|null, interactionType: InteractionType, isLoginCall: boolean, request: AuthenticationParameters, resolve?: any, reject?: any): Promise<void> {
        ...
        try {
            ...
            if (interactionType === Constants.interactionTypeRedirect) {
                ...
                this.registerCallback(serverAuthenticationRequest.state, scopeString, resolve, reject);
            } else {
                throw ClientAuthError.createInvalidInteractionTypeError();
            }
    ...
    acquireTokenSilent(userRequest: AuthenticationParameters): Promise<AuthResponse> {
        ...
        return new Promise<AuthResponse>(async (resolve, reject) => {
            ...
            let authErr: AuthError|undefined;
    ...
    protected saveTokenFromHash(hash: string, stateInfo: ResponseStateInfo): AuthResponse {
        ...
        let response : AuthResponse = buildResponseStateOnly("");
        ...
        // If the server returns "Success"
            ...
            if (stateInfo.stateMatch) {
                ...
                if (hashParams.hasOwnProperty(ServerHashParamKeys.ACCESS_TOKEN)) {
                    ...
                    const cachedAccount: string|null = this.cacheStorage.getItem(acquireTokenAccountKey);
    ...
    getAccount(): Account|null {
        ...
        const rawIdToken : string|null = this.cacheStorage.getItem(PersistentCacheKeys.IDTOKEN);
        const rawClientInfo : string|null = this.cacheStorage.getItem(PersistentCacheKeys.CLIENT_INFO);
     ...
     * @param endpoint
     */
    protected getScopesForEndpoint(endpoint: string) : Array<string>|null {
     ...
     *
     */
    private getTokenType(accountObject: Account|null, scopes: string[], silentCall: boolean): string {
        /*
