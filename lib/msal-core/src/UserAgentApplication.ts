...
import { Constants, InteractionType, libraryVersion, ResponseTypes } from "./utils/Constants";
...
export class UserAgentApplication {
    ...
    private acquireTokenHelper(serverAuthenticationRequest: ServerRequestParameters, resolve?: any, reject?: any): void {
        ...
        serverAuthenticationRequest.authorityInstance.resolveEndpointsAsync().then(async () => {
            // On Fulfillment
    ...
    acquireTokenSilent(request: AuthenticationParameters): Promise<AuthResponse> {
        ...
        return new Promise<AuthResponse>((resolve, reject) => {
            ...
            // populate QueryParameters (sid/login_hint/domain_hint) and any other extraQueryParameters set by the developer
            if (serverAuthenticationRequest.isSSOParam() || account) {
                serverAuthenticationRequest.populateQueryParams();
            }
            // if user didn't pass login_hint/sid and adal's idtoken is present, extract the login_hint from the adalIdToken
                ...
                serverAuthenticationRequest.populateQueryParams(adalIdTokenObject);
            }
    ...
    private getCachedToken(serverAuthenticationRequest: ServerRequestParameters, account: Account): AuthResponse {
        ...
        if (!serverAuthenticationRequest.authority) {
            ...
            for (let i = 0; i < tokenCacheItems.length; i++) {
                ...
                if (cachedScopes.containsScopeSet(scopes)) {
        ...
        // if an authority is passed in the API
            ...
            for (let i = 0; i < tokenCacheItems.length; i++) {
                const cacheItem = tokenCacheItems[i];
                const cachedScopes = ScopeSet.fromString(cacheItem.key.scopes, this.clientId, false);
                if (cachedScopes.containsScopeSet(scopes) && UrlUtils.CanonicalizeUri(cacheItem.key.authority) === serverAuthenticationRequest.authority) {
                    filteredItems.push(cacheItem);
                }
            }
            // no match
    ...
    private renewToken(scopes: Array<string>, resolve: Function, reject: Function, account: Account, serverAuthenticationRequest: ServerRequestParameters): void {
        ...
        const urlNavigate = UrlUtils.urlRemoveQueryStringParameter(serverAuthenticationRequest.createNavigateUrl(), Constants.prompt) + Constants.prompt_none;
        ...
        if (parameters.hasOwnProperty("scope")) {
            ...
            for (let i = 0; i < accessTokenCacheItems.length; i++) {
                ...
                if (accessTokenCacheItem.key.homeAccountIdentifier === response.account.homeAccountIdentifier) {
                    ...
                    if (cachedScopes.intersectingScopeArrays(consentedScopes)) {
