...
export abstract class BaseApiClient {
    ...
    private readResponseCorrelationId(response: Response, requestCorrelationId: string): string {
        return response.headers.get(HttpHeaderKeys.X_MS_REQUEST_ID) || requestCorrelationId;
    }
    ...
    private async handleApiResponse<T>(response: Response | undefined, requestCorrelationId: string): Promise<T> {
        ...
        if (response.ok) {
            ...
            if (typeof responseData === "object" && responseData.challenge_type === ChallengeType.REDIRECT) {
        ...
        const attributes =
            !!responseError.required_attributes && responseError.required_attributes.length > 0
