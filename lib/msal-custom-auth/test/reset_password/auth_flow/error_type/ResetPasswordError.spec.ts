...
} from "../../../../src/reset_password/auth_flow/error_type/ResetPasswordError.js";

describe("ResetPasswordError", () => {
    it("should correctly identify user not found error", () => {
        const error = new CustomAuthApiError("user_not_found", "User not found");
...
describe("ResetPasswordSubmitPasswordError", () => {
    it("should correctly identify invalid password error", () => {
        ...
        const resetPasswordError2 = new ResetPasswordSubmitPasswordError(error2);
        expect(resetPasswordError2.isInvalidPassword()).toBe(true);
    });

    it("should correctly identify password reset failed error", () => {
        const error1 = new CustomAuthApiError("password_reset_timeout", "Password reset timeout");
        const resetPasswordError1 = new ResetPasswordSubmitPasswordError(error1);
        expect(resetPasswordError1.isPasswordResetFailed()).toBe(true);

        const error2 = new CustomAuthApiError("password_change_failed", "Password reset is failed");
        const resetPasswordError2 = new ResetPasswordSubmitPasswordError(error2);
