...
describe("ResetPasswordPasswordRequiredStateHandler", () => {
    ...
    let handler: ResetPasswordPasswordRequiredStateHandler;

    beforeEach(() => {
        handler = new ResetPasswordPasswordRequiredStateHandler(
            correlationId,
            mockLogger,
            continuationToken,
            mockConfig,
    ...
    describe("submitPassword", () => {
        it("should return an error result if password is empty", async () => {
            const result = await handler.submitNewPassword("");
        ...
        it("should successfully submit a password and return completed state", async () => {
            ...
            const result = await handler.submitNewPassword("valid-password");

            expect(result).toBeDefined();
            expect(result).toBeInstanceOf(ResetPasswordSubmitPasswordResult);
            expect(result.state?.type).toBe(ResetPasswordState.Completed);
            expect((result.state as ResetPasswordCompletedResult)?.continuationToken).toBe("new-continuation-token");
