...
│   ├── auth_flow/                       # Base auth flow classes and MFA components
│   │   └── mfa/                         # MFA-specific auth flow components
│   │       ├── error_type/              # MFA error types
│   │       ├── result/                  # MFA result types
│   │       └── state/                   # MFA state types
...
│   └── mfa/                            # MFA-specific interaction clients
│       ├── MfaClient.ts                # MFA interaction client
│       ├── parameter/                  # MFA parameter types
│       └── result/                     # MFA result types
├── network_client/                     # HTTP and API clients
...
1. **Add shared flow client** to the existing flow's state parameters (e.g., `mfaClient: MfaClient` in `SignInStateParameters`)
2. **Add result checking methods** to the existing flow result (e.g., `isMfaRequired()`, `isRegistrationRequired()`)
...
5. **Add shared flow result types** to existing flow action results (e.g., `SignInMfaRequiredResult`, `SignInRegistrationRequiredResult`)
